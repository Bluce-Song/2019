    ...
    ui(new Ui::w_Test)
{
    ...
    TaskMagDirnumber = 0;
    TaskMagDirnumber_ME = 0;
    TaskMagDirnumber_YY = 0;
    TaskMagDirnumber_YY_ROT = 0;
    Data_Complete = false;
    StrResult_Size = 0;
    StrJudge_Size  = 0;
    ...
    Ini_W_Connext     = set_Sys->value("DeChang/Connect", "0").toInt();
    Ini_DeChang       = set_Sys->value("DeChang/Enable", 0).toInt();
    Ini_Discharge     = set_Sys->value("DeChang/Discharge", 0).toInt();
    Ini_IMP_36        = set_Sys->value("System/IMP_36", "0").toInt();
    Ini_Get_Temp      = set_Sys->value("System/Temp", "Inter").toString(); // 飛雪派克外部溫度
    ACW_Set_Point     = set_Sys->value("ACW_Set/Point", 3).toInt(); // 耐压显示２位小数点
    Int_Diaolay_number = set_Sys->value("System/Display_number", 0).toInt(); // 显示合格与不合格
    Ini_ROT       = false;

    if (Int_Diaolay_number) {
//        ui->label_sum->hide();
        ui->label_sum->setGeometry(675, 260, 115, 30);
        ui->label_qualified->hide();
        ui->label_Unqualified->hide();
//        ui->button_shape->hide();
    } else {
        //
    }
    QString magtext;
    magtext = set_Sys->value(QString("MAG/text").toUtf8().data()).toString();
    if (magtext == QString("")) {
        //
    } else {
    ...
    False_Test  = false;
    BEMF_Count = 0;
    Turn_Elec_force_List.clear();
    Turn_Elec_forceing = false;
    DCR_Connect = false;
    ...
    WL_Start = new QTimer(this);
    connect(WL_Start, SIGNAL(timeout()), this, SLOT(WL_Start_timeout()));

    PG_Start_Start = new QTimer(this);
    connect(PG_Start_Start, SIGNAL(timeout()), this, SLOT(PG_State_timeout()));
    In_Temp       = 0; // -机箱内温度
    ...
    DCW_Error.clear();
    DCW_Error.append("1");        // -1
    DCW_Error.append("2");        // -2
    DCW_Error.append("3");        // -3
    DCW_Error.append("4");        // -4
    DCW_Error.append("LEAK");     // -5
    DCW_Error.append("OVER-POWER"); // -6
    DCW_Error.append("DCPT");     // -7
    DCW_Error.append(">20mA");    // -8
    ...
    NetTest_Ready_DCW = false;     DCW_Issue_Ing = false;
    ...
    NetTest_Resdy_PWR = false;     PWR_Issue_Ing = false;
    NetTest_Resdy_HV  = false;     HV_Issue_Ing = false;
    ...
    Test_Message.append(QString(tr("  系统接地不良     \n  请先检查地线再进行测试 ")));  // 4
    ...
    Test_Message.append(QString(tr("光幕被遮挡,退出启动"))); // 12
    ...
    connect(Noise_Stop, SIGNAL(timeout()), \
            this, SLOT(Noise_Stop_timeout()));
    Noise_Blink = new QTimer(this);
    connect(Noise_Blink, SIGNAL(timeout()), \
            this, SLOT(Noise_Blink_timeout()));
    count_Noise_Blink = 0;
    ...
    Ciximinghua_Old = 0;
    Test_Curtain = true;
    IND_Balance_Error = false;

    DC_IO_Put = 0;
    W_Connext = true;
    Current_Test_Window = false;
    ACW_Result_DeChang = (QString::number(0.000));
    DeChang_Connect_Time = 0;
    Back_ACW_Result = false;
...
void w_Test::Prompt_Windows() {
    Prompt_Widget = new QWidget(this);
    Prompt_Widget->setGeometry(670, 188, 122, 235);
    Prompt_Widget->setWindowFlags(Qt::WindowStaysOnTopHint);
    Prompt_Widget->setStyleSheet("border-radius: 10px;padding:2px 4px;background-color: gray;");
    QGridLayout *Prompt_Up = new QGridLayout;
    Prompt_Up_label = new QLabel(this);
    Prompt_Up_label->setText(tr(""));
    Prompt_Up_label->setAlignment(Qt::AlignCenter);
    Prompt_Up->addWidget(Prompt_Up_label, 0, 0);
    QVBoxLayout *Prompt_Widget_layout = new QVBoxLayout;
    Prompt_Widget_layout->addLayout(Prompt_Up);
    Prompt_Widget->setLayout(Prompt_Widget_layout);
    Prompt_Widget->hide();
}

void w_Test::Test_Ask_Widget() {
    /******************询问和提示显示窗口******************/
...
void w_Test::Test_Histogram_Widget() {
    int i;
    /******************柱状图显示窗口******************/
...
void w_Test::ICO_Show_Widget() {
    PLC_Singal_Widget = new QWidget(this);
    ...
    Motor_label = new QLabel; // 电机到位图标
    Motor_label->setPixmap(QPixmap(":/image/stepper_motor.png"));
    PLC_upside->addWidget(Motor_label, 0, 0);
    Motor_label->hide();

    COM_label = new QLabel; // COM连接图标
    COM_label->setText("COM");
    COM_label->setStyleSheet("font-size:12pt; color:#0055ff; font: bold; ");
    PLC_upside->addWidget(COM_label, 0, 1);
    COM_label->hide();

    QHBoxLayout *hb_one = new QHBoxLayout;
    hb_one->addStretch();
    PLC_upside->addLayout(hb_one, 0, 8);
...
{
    qDebug() << "w_Test::InitTest_Interface";
    int i = 0;
    QString str;
    QString Plan_Number;
    ...
    TaskMagDirstrList[3].clear();
    TaskMagDirstrList[4].clear();
    TaskMagDirstrList[5].clear();
...
void w_Test::Noise_Blink_timeout() {
    count_Noise_Blink++;
    qDebug() << count_Noise_Blink;
    if (count_Noise_Blink%2) {
        qDebug() << "1";
        ui->label_noise->setPixmap(QPixmap(":/image/sound_file.ico"));
    } else {
        qDebug() << "2";
        ui->label_noise->setPixmap(QPixmap(":/image/sound_5.png"));
    }
    if (count_Noise_Blink >= 10000) {
        count_Noise_Blink = 0;
    } else {
        //
    }
...
void w_Test::Noise_Stop_timeout() {
    if (Noise_step == 0) {
        if (strPWR.at(24).toInt()  == 2) { // 反向测试
            PLC_test_to_Switchover(Noise, Station_Chose - 0x13 + 1, 7); // 切换进入反向测试
            Noise_step = 1;
            ...
            PLC_test_to_Switchover(Noise, 0x00, 7); // 关闭所有通路
            ui->label_noise->hide();
            Noise_Blink->stop();
        }
    } else if (Noise_step == 1) {
        ui->label_noise->show();
        PLC_test_to_Switchover(Noise, Station_Chose - 0x13 + 1 + 16, 7); // 切换进入反向测试
        Noise_step = 2;
        Noise_Stop->setSingleShot(TRUE);
        Noise_Stop->start(Noise_CCW_Time*1000); //
        ui->label_noise->show();
        Noise_Blink->start(300);
...
void w_Test::Pubs_from_pic(QString string_c, int value, int flag)
{
    switch (flag) {
    case 1:
    ...
    case 102:
        ...
        if (Test_MAG_Ready) {
            ...
            Can_Test_MAG_Parameter();
            Can_Start_MAG();
        }
        break;
    case 103:
        Vacuum_Ready = false;
        if (Test_Model_Exhaust_ACW) {
            if (Test_ACW_Ready) {
                Test_ACW_Ready = false;
                NetTest_Ready_ACW = true;
                Can_Start_ACW(1);
...
void w_Test::join_updateTime()
{
    ...
    if (Test.Status_Ing == Test_Station_Free) {
        if (Current_Test_Window) {
            System_Free_Time++;
            if (System_Free_Time >= 100) { // 10min 200 1min 20
                System_Free_Time = 0;
                Vacuum_pump_Close = true;
                if ((Test_Model_Vacuum) || (Test_Model_Vacuum_ACW)) {
                    qDebug() << "w_Test::join_updateTime() Close Pumb";
                    Signal_test_to_pic(QStringList(""), 0, 12); // 关闭真空泵 真空增加
                    WriteMessage(6000, CMD_Put_Close_Pumb, test_parameters[3]->text().toUtf8());
                } else {
                    //
                }
            } else {
    ...
    if ((Ini_DeChang) && (Current_Test_Window)) {
        DeChang_Connect_Time++;
        Sigtest_Usart_Send_DeChang(QString("70  3 ")); // 联机查询
        if (DeChang_Connect_Time > 5) {
            DeChang_Connect_Time = 0;
            COM_label->hide();
        } else {
            //
    ...
    if (Test.Status_Ing == Test_Station_Free) {
        ...
        if (Ini_Get_Temp == "External") {
            GetTemp_ing = "?T\r";
            WriteMessage_NoVacuum(6000, CMD_In_GetTemp, GetTemp_ing.toUtf8());
        } else {
            //
        }
    ...
    for (int i = 0; i < PG_Row.size(); i++) {
        ...
        if (ui->testTab->item(Current_Row, 2)->text().contains(QString(tr("Hz")))) {
            pItem->setText(strParam.at(Current_Row).\
                           mid(3 + strParam.at(Current_Row).indexOf(QString(tr("Hz")))));
            ui->testTab->setItem(Current_Row, 2, pItem);
            if ((i <= PG_Result_List.size()) && \
                    (ui->testTab->item(Current_Row, 3)->text() != NULL)) {
                pItem3->setText(PG_Result_List.at(i).\
                                mid(3 + PG_Result_List.at(i).indexOf(QString(tr("Hz")))));
...
void w_Test::Board_DCW_Data(QStringList DCW_Data_List) {
    QString str;
    double  Replace_Data = 0.0;
    if ((DCW_Data_List.isEmpty()) || (DCW_Data_List.size() != 3)) {
        DCW_Data_List.clear();
        DCW_Data_List.append("--");
        DCW_Data_List.append("--");
        DCW_Data_List.append("1");
    }
    RemoveLastIR = true;

    if (Ini_ACWandIR) {
        Replace_Data = DCW_Data_List.at(1).toDouble() - strDCW.at \
                (DCWgroup_num.at(DCW_Test_Count).toInt()*16 + 37).toDouble();
        if (Replace_Data < 0.00) {
            Replace_Data = 0.00;
        }
        DCW_Data_List.replace(1, QString::number(Replace_Data));
    }

    str = DCW_Data_List.at(0) + \
            (QString::number(DCW_Data_List.at(1).toDouble(), 'f', ACW_Set_Point)) + "mA";
    ...
    RemoveLastIR = false;
    switch (DCW_Data_List.at(2).toInt()) {
    case 0: str = "OK";
        break;
    ...
    default:
        ...
        strResult.append(DCW_Error.at(DCW_Data_List.at(2).toInt()));
        str = "NG";
        Test.NG_Item = 4;
        break;
    }
    strJudge.append(str);  str.clear();
    IRFinsh = false; qDebug() << "w_Test::Board_DCW_Data Test DCV Ok";
    if (Ini_ACWandIR) {
        DCW_Test_Count++;
        if (DCW_Test_Total == DCW_Test_Count) {
            DCW_Test_Count = 0;
            Test.Finshed_Count++;
        } else {
            Test.NG_Item = 0;
            if (Test.Interrupted) {
                ...
                if (Model_Occur_Unqualified == Test_NG_Continue) { // -遇不合格测试项目继续
                    ...
                    Can_Start_DCW();
                           ...
                           (DCW_Data_List.at(2).toInt())) { // -遇不合格测试项目停止
                    join_test_stop(3);
                } else if ((Model_Occur_Unqualified == Test_NG_Ask) && \
                           (DCW_Data_List.at(2).toInt())) { // -遇不合格测试项目询问
...
void w_Test::Board_HV_Data(QStringList HV_Data_List) {
    QString str;
    double  Replace_Data = 0.0;
    if ((HV_Data_List.isEmpty()) || (HV_Data_List.size() != 3)) {
        HV_Data_List.clear();
        HV_Data_List.append("--");
        HV_Data_List.append("--");
        HV_Data_List.append("1");
    }
    RemoveLastIR = true;

    str = HV_Data_List.at(0) + \
            (QString::number(HV_Data_List.at(1).toDouble(), 'f', ACW_Set_Point)) + "mA";
    strResult.append(str);
    str.clear();
    if (!IRFinsh) {
        HV_Data_List.clear();
    ...
    RemoveLastIR = false;
    switch (HV_Data_List.at(2).toInt()) {
    case 0: str = "OK";
        ACW_Result_DeChang = (QString::number(HV_Data_List.at(1).toDouble(), 'f', ACW_Set_Point));
        break;
    case 1: str = "NG";  Test.NG_Item = 4;
        ACW_Result_DeChang = (QString::number(HV_Data_List.at(1).toDouble(), 'f', ACW_Set_Point));
        break;
    ...
    default:
        strResult.removeLast();
        strResult.append(ACW_Error.at(HV_Data_List.at(2).toInt()));
        ACW_Result_DeChang = (QString::number(0.000));
        str = "NG";
        Test.NG_Item = 4;
        break;
    ...
    if (str == "OK") {
        ...
        Sigtest_Usart_Send_DeChang(QString("69  7 "));
    }
    str.clear();
    IRFinsh = false; qDebug() << "w_Test::Board_ACW_Data Test ACV Ok";
    if (Ini_ACWandIR) {
        //
    } else {
        Test.Finshed_Count++;
...
void w_Test::Board_ACW_Data(QStringList ACW_Data_List)
{
    ...
    str = ACW_Data_List.at(0) + \
            (QString::number(ACW_Data_List.at(1).toDouble(), 'f', ACW_Set_Point)) + "mA";
    strResult.append(str);
    str.clear();
    if (!IRFinsh) {
        ACW_Data_List.clear();
    ...
    case 0: str = "OK";
        ACW_Result_DeChang = (QString::number(ACW_Data_List.at(1).toDouble(), 'f', ACW_Set_Point));
        break;
    case 1: str = "NG";  Test.NG_Item = 4;
        ACW_Result_DeChang = (QString::number(ACW_Data_List.at(1).toDouble(), 'f', ACW_Set_Point));
        break;
...
void w_Test::Board_IR_Data(QStringList IR_Data_List)
{
    ...
    IR_Data_int = IR_Data_List.at(1).toDouble();
    RemoveLastIR = true;
    if (IR_Data_List.at(1).toDouble() > 100) {
        ...
        str = IR_Data_List.at(0) + (QString::number \
                                    (IR_Data_List.at(1).toDouble(), 'f', 1)) + "MΩ";
    }
    ...
    if (str == "OK") {
        ...
        Sigtest_Usart_Send_DeChang(QString("69  5 "));
    }
    str.clear();

    IRFinsh = false;  qDebug() << "w_Test::Board_IR_Data Test IR Ok";
    if (Ini_ACWandIR) {
        IR_Test_Count++;
...
void w_Test::Board_PWR_Data()
{
    ...
    if ((Ini_Motor == QString(tr("PG"))) && (strPWR.at(5 + PWR_Data).toInt() == 2)) { // -显示PG
        qDebug() << "w_Test::Board_PWR_Data() PG_list" << PG_list;
        if (PG_list.size() != 6) {
            PG_list.clear();
            PG_list.append("0");
            PG_list.append("0");
        ...
        strResult.append(str);
        qDebug() << "PG_list PG_Result_List " << PG_Result_List;
    ...
    if (PWR_Test_Data.at(0).toInt() == PWR_Test_Count) {
        //        PLC_test_to_Switchover(MaiXing, 0x12, 13);
        PLC_test_to_Switchover(Vacuum_Cooperate, ((1 << 4) | (1 << 5) | (1 << 7)), 1);
        Test.Finshed_Count++; // -增加计数
    } else {
        PC_Test_Result_Data(" ", QString(tr("Test_Data_Result")));
        PC_Test_Result_ItemData(QString::number(PWR_Test_Data.at(0).toInt()), "PWR");
        if (Test.Interrupted) {
            Test.Interrupted = false;
            join_test_stop(3);
        } else {
...
{
    QString str;
    qDebug() << "w_Test::Board_IMP_Data" << IMP_Data_List;
    ...
    if (Data_Complete) {
        ...
        qDebug() << "w_Test::Board_IMP_Data IMP_Text" << IMP_Text;
    ...
    if (IMP_Total_Number  ==  IMP_Test_Record_Number) {
        ...
        qDebug() << "w_Test::Board_IMP_Data Test ITT Ok";
        Test.Finshed_Count++;
        if ((Ini_IMP_36) && (strITT.at(13).toInt() == 2)) {
            PLC_test_to_Switchover(BaoJiaLiYa, 18, 2);
            usleep(10000); usleep(10000); usleep(10000);
        } else {

        }
        if (Test_Model_Exhaust_ACW) {
            Signal_test_to_pic(QStringList(""), Station_Chose, 23);
        }
        PLC_test_to_Switchover(BDVacuum, 0, 9);
        PLC_test_to_Switchover(Vacuum_Cooperate, ((1 << 4) | (1 << 7)), 1);
    }
}
...
void w_Test::Board_BLOCK_Data()
{
    ...
    if (Ini_Set_pwr_Turn) {
        PLC_test_to_Switchover(MaiXing, 0X18, 13);
    } else {
        //
    }
    strJudge.append(str);
    Test.Finshed_Count++;
    //    Kongshi_Stall = true;
    //    PLC_test_to_Switchover(ZhongTe, ((1 << 0)*65536) | (1 << 7), 1);
    qDebug() << "w_Test::Board_BLOCK_Data() BLOCK Finsh";
}
...
void w_Test::Board_INDL_Unbalance(int indl_count) {
    double  INDL_Balance_All = 0.0, INDL_Balance_Average = 0.0, \
            INDL_Balance_Value = 0.0, INDL_Balance_Set = 0.0;
    double  Res_Balance_Value = 0.0;
    ...
    double  INDL_Balance_difference = 0.0; // -電感不平衡度两两数值差
    if ((strINDL.at(10).toInt() == 1)&&(indl_count >= 3)) {
        ...
        for (i = 0; i < 3; i++) {
            INDL_Balance_Value = \
                    fabs((fabs(INDL_Balance_Average-ResultTable_INDL_All[i+1]) \
                         *100/INDL_Balance_Average));
            // .replace()
            if (INDL_Balance_Value > 100.0) {
                INDL_Balance_Value = 100.0;
            }

            if (IND_Balance_Error) {
                INDL_Balance_List += (QString(tr("--")) + "% ");
                INDL_Balance_Judge = 1;
            } else {
                INDL_Balance_List += (QString::number(INDL_Balance_Value, 'f', 3) + "% ");
            }
    ...
    } else if ((strINDL.at(10).toInt() == 2) && (indl_count >= 2)) { // -两两相减的电阻差
        INDL_Balance_Set = strINDL.at(1).toDouble();
        INDL_Balance_difference = 0;
        x = 0;
        y = 0;
        for (j = strResult.size(); j > strResult.size() - indl_count; j--) {
            for (i = strResult.size(); i > strResult.size() -indl_count; i--) {
                Compare_One =  ResultTable_INDL_All[x + 1];
                Compare_Two =  ResultTable_INDL_All[y + 1];
                qDebug() << ResultTable_INDL_All[x + 1] << ResultTable_INDL_All[y + 1];
                Compare_Finsh = fabs(Compare_One-Compare_Two);
                if (Compare_Finsh > INDL_Balance_difference) {
                    INDL_Balance_difference = Compare_Finsh;
                }
                x++;
            }
            x = 0;
        ...
        Res_Balance_Value = INDL_Balance_difference*100/strINDL.at(36).toDouble();
        Res_Balance_List += (QString::number(Res_Balance_Value, 'f', 3)+"% ");
        if (Res_Balance_Value > INDL_Balance_Set) {
            Res_Balance_Judge = 1;
        }
        ...
        Test.Current_Row++; // -行数增加
    } else if ((strINDL.at(10).toInt() == 3)&&(indl_count >= 2)) {
        INDL_Balance_Set = strINDL.at(1).toDouble();
        INDL_Balance_difference = 0;
        x = 0;
        y = 0;
        ...
        Res_Balance_List += (QString::number(INDL_Balance_difference, 'f', 3));
        if (INDL_Balance_difference > INDL_Balance_Set) {
            Res_Balance_Judge = 1;
        }
        strResult.append(Res_Balance_List);
        if (Res_Balance_Judge == 1) {
        ...
        Test.Current_Row++; // -行数增加
    } else if ((strINDL.at(10).toInt() == 0)&&(indl_count >= 3)) {
        INDL_Balance_Set = strINDL.at(1).toDouble();
        for (i = 0; i < 3; i++) {
            INDL_Balance_All += ResultTable_INDL_All[i + 1];
        }
...
void w_Test::Board_INDL_Data()
{
    int i = 0;
    QString str;
    double  Q_Balance_All = 0.0, Q_Balance_Average = 0.0, \
            Q_Balance_Value = 0.0, Q_Balance_Set = 0.0;
    ...
    if (str == QString(tr("NG"))) {
        ...
        if (strINDL.at(7) == "0.0") {
            //
        } else if (strINDL.at(7) == "0.000") {
            //
        } else {
            if (INDL_Total_Number >= 3) {
    ...
    if (INDL_Test_Record_Number <= 8) { // -计算不平衡度使用
        if (strINDL.at(30 + 20*(Result_INDL - 1) + 7).toInt()  ==  0) {
            ResultTable_INDL_All[INDL_Test_Record_Number] = \
                    QString::number(Result_Main, 'f', 1).toDouble();
        } else {
            ResultTable_INDL_All[INDL_Test_Record_Number] = \
                    QString::number(Result_Main/1000, 'f', 3).toDouble();
        }
        qDebug() << Result_Main << ResultTable_INDL_All[INDL_Test_Record_Number] << "8888888888888";
    ...
    if (strINDL.at(1) == "0.000") {
        //
    } else if (strINDL.at(1) == "0.0") {
        //
    } else {
        Board_INDL_Unbalance(INDL_Total_Number);
    }
    ...
    if (INDL_Total_Number >= 3) {
        if (strINDL.at(7) == "0.000") {
            //
        } else if (strINDL.at(7) == "0.0") {
            //
        } else {
    ...
    qDebug() << "w_Test::Board_INDL_Data() Test INDL Ok";
    Test.Finshed_Count++;
}
...
void w_Test::Board_MAG_Data(QStringList MAG_Data_List)
{
    ...
    if (TaskMagDirnumber != 5) { // -判断是否进行磁旋的判定
        qDebug() << "w_Test::Board_MAG_Data --> " << Test.NG_Item << TaskMagDirnumber;
        if (Test.NG_Item == 2) {
            TaskMagDirnumber = 2;
        } else {
            //
        ...
        if (YY_ROT_Test) {
            ...
            strResult.append(str);
            if (strOPP.at(6563).toInt() == TaskMagDirnumber_YY) {
                str = "OK";
            } else {
                str = "NG";
                Test.NG_Item = 2;
        ...
        if (MEKJ_Test) {
            ...
            strResult.append(str);
            if (strOPP.at(6563).toInt() == TaskMagDirnumber_ME) {
                str = "OK";
            } else {
                str = "NG";
                Test.NG_Item = 2;
...
void w_Test::Board_DLR_Data(QStringList DLR_Data_List)
{
    ...
    if ((DLR_Data_List.size() == 1) && (DLR_Data_List.at(0) == QString(tr("Error")))) {
        ...
        Test_Net_WMessage("Sensor Error");
        return;
    }
    if ((DLR_Data_List.isEmpty()) || (DLR_Data_List.size() != 5)) { // 7
        DLR_Data_List.clear();
...
//        DLR_Data_List.append("0");
    ...
    strResult.append(str); // DLR_Data_List.at(5)+ DLR_Data_List.at(6) + "   " +
    ...
    if (((DLR_Data_List.at(0).toDouble()) >= \
        ...
        if (((DLR_Data_List.at(0).toDouble())) <= \
            ...
            qDebug() << (DLR_Data_List.at(0).toDouble()) <<
                        (DLR_UpDown.at(DLR_Data_List.at(2).toInt()*2 + 1).toDouble()/2);
            DCR_Connect = true;
        } else {
            DCR_Connect = false;
    ...
    if (DCR_Total_Number == DCR_Test_Record_Number) { // -判断电阻是否测试完成
        ...
        qDebug() << "w_Test::Board_DLR_Data() Test DLR Ok";
        Test.Finshed_Count++;
    }
}
...
{
    double  Res_Balance_All = 0.0, Res_Balance_Average = 0.0, \
            Res_Balance_Value = 0.0, Res_Balance_Set = 0.0;
    ...
    if ((strRES.at(10).toInt() == 1)&&(DCR_Total_Number >= 3)) { // -普通电阻三个测试不平衡度
        qDebug() << "dlr_conut" << dlr_conut;
        x = 0;
        for (i = strResult.size() - dlr_conut; (i < strResult.size() && (x<3)); i++) {
            x ++;
            qDebug() << strResult.at(i);
            Res_Balance_All += QString(strResult.at(i)).replace \
                    (QRegExp("[^\\d.]+"), "").toDouble(); // .replace()
        }
        Res_Balance_Average = Res_Balance_All/3;
        qDebug() << "Res_Balance_Average" << Res_Balance_Average;
        ...
        if ((YY_Model) && (DCR_Total_Number >= 6)) {
            Res_Balance_Judge = 0;
            Res_Balance_All = 0;
            for (i = strResult.size() - dlr_conut + 2; (i < strResult.size() && (i<6)); i++) {
                qDebug() << "strResult.at(i)" << strResult.at(i);
                Res_Balance_All += QString(strResult.at(i)).replace \
                        (QRegExp("[^\\d.]+"), "").toDouble(); // .replace()
            }
            Res_Balance_Average = Res_Balance_All/3;
        ...
        Test.Current_Row++; // -行数增加
    } else if ((strRES.at(10).toInt() == 3) && (DCR_Total_Number >= 2)) { // -两两相减的电阻差
        Res_Balance_difference = 0;
        double Compare_One = 0.0, Compare_Two = 0.0;
        double Compare_Finsh = 0.0;
        for (j = strResult.size(); j > strResult.size() - dlr_conut; j--) {
        ...
        if (Res_Balance_difference > Res_Balance_Set) {
            Res_Balance_Judge = 1;
        }
        strResult.append(Res_Balance_List);
        if (Res_Balance_Judge == 1) {
...
void w_Test::Board_FG_Data(QStringList Updatedata_List) {
    ...
    FG_list.append(QString::number\
                   ((Updatedata_List.at(20 + 4*FG_Test_Count).toDouble()/1000), 'f', 1));
    FG_list.append(QString::number\
                   ((Updatedata_List.at(21 + 4*FG_Test_Count).toDouble()/1000), 'f', 1));
    ...
    strJudge.append(str);
    //  emit WriteMessage(6000, CMD_Put_73, QString(Updatedata_List.join(" ")).toUtf8());
    ...
    FG_Test_Count++;
    qDebug() << "w_Test::Board_FG_Data" << FG_Test_Data.at(FG_Test_Count);
...
void w_Test::Board_NOLOAD_Data(QStringList Updatedata_List) {
    ...
    Updatedata_List.replace(23, QString::number((Updatedata_List.at(23).toInt()*60/1000) \
                                               /strNOLOAD.at(13).toInt()));

    Noload_list.clear();
    if (Station_Chose == 0x13) {
        I_get = Updatedata_List.at(20).toDouble() + strNOLOAD.at(15).toDouble();
    } else {
        I_get = Updatedata_List.at(20).toDouble() + strNOLOAD.at(16).toDouble();
    }
    ...
    P_get = I_get * Updatedata_List.at(21).toDouble();
    Noload_list.append(QString::number(I_get, 'f', 2));
    Noload_list.append(QString::number(Updatedata_List.at(21).toDouble(), 'f', 1));
    Noload_list.append(QString::number(P_get, 'f', 1));
    Noload_list.append(QString::number(Updatedata_List.at(23).toInt()));

    qDebug() << "11";
    if (Updatedata_List.at(26).toInt() > 2) {
        Updatedata_List.replace(26, QString::number(0));
    }
    ...
    if (strNOLOAD.at(11).toInt() != 0) {
        str += ",";
        str += FG_direction.at(Updatedata_List.at(26).toInt());
    }
    qDebug() << "11";
    strResult.append(str);
    if (!NoLoadFinsh) {
        qDebug() << "w_Test::Board_NOLOAD_Data return";
        return;
    } else {
        //
    }
    ...
    qDebug() << "11";
    qDebug() << "w_Test::Board_NOLOAD_Data";
    for (i = 0; i < 4; i++) {
    ...
    if ((strNOLOAD.at(11).toInt() != 0) && (str == "OK")) {
        if (Updatedata_List.at(26).toInt() != strNOLOAD.at(11).toInt()) {
            str = "NG";
            Test.NG_Item = 13;
        } else {
            //
    ...
    PLC_test_to_Switchover(Bosch, 0x01, 7);
}
...
void w_Test::join_updatedata(QStringList Updatedata_List)
{
    ...
    if (Test.Status_Ing != Test_Station_Busy) { // -在测试遇不合格测试项目时
        qDebug() << "w_Test::join_updatedata  Not test busying,exit test" << Test.Status_Ing;
        return;
    }
    qDebug() << "w_Test::join_updatedata";
    ...
    case 4: // -交耐
        ...
        break;
    case 5: // -直耐
        Board_DCW_Data(Updatedata_List);
        break;
    case 6: // -匝间
    ...
    case 7: // -功率
        Board_PWR_Data();
        PLC_test_to_Switchover(NanXin, 18, 5);
        PLC_test_to_Switchover(NANXIN_Vacuum, 18, 17);
        break;
    case 8: // -电感
        Board_INDL_Data();
        break;
    ...
    for (i = StrJudge_Size; i < strJudge.size(); i++) {  // -添加测试判定
        ...
        qDebug() << "w_Test::join_updatedata--" << i << strJudge[i] << strResult[i];
    ...
    if (Conf_Item.contains(tr("7")) || \
        ...
        if (DCR_Connect) {       // -测试线圈是否短路,进行保护
            ...
            Prompt_Up_label->setAlignment(Qt::AlignCenter);
            Prompt_Up_label->setText(tr("电阻过小\nSTOP"));
            Test_Net_WMessage("DCR Small,STOP");
            Prompt_Widget->show();
        } else {
            //
        }
    ...
    if (Test.Next_Finsh_Count != Test.Finshed_Count) { // -测试单个项目和总项目是否完成
        ...
        case 4: // -交耐
            ...
            break;
        case 5: // -直耐
            PC_Test_Result_ItemData(QString::number(1), "DCW");
            break;
        case 6: // -匝间
    ...
    qDebug() << "w_Test::join_updatedata -----" << (!(Test.Interrupted));
    qDebug() << "w_Test::join_updatedata ------" << Test.Judge_NG;
    qDebug() << "w_Test::join_updatedata -------" << \
    ...
    if ((!(Test.Interrupted)) && (Test.Judge_NG) && (Model_Occur_Unqualified == Test_NG_Ask) \
        ...
        Test_SelfStart();
        qDebug() << "w_Test::join_updatedata Test All Ok";
    } else { // -测试单个完成，触发下一个项目
        qDebug() << "w_Test::join_updatedata ---------" << Test.Interrupted \
                 << Test.Judge_NG << Model_Occur_Unqualified;
        if (Test.Interrupted) {
            Test.Interrupted = false;
            join_test_stop(3);
            lable_stop->show();
...
void w_Test::IO_Pumb_Start_timeout() {
    ...
    Test_Net_WMessage("Air Cylinder Overtime");
}
...
void w_Test::on_button_set_clicked()
{
    ...
    qDebug() << "w_Test::on_button_set_clicked()" << JUMP_Chose;
    IO_Test_Readying = false;
...
void w_Test::Test_CiXiMingHua() {
    if (Ini_IO_Model == Ciximinghua) {
        qDebug() << "w_Test::Test_CiXiMingHua()"\
                 << Test.Finshed_Count << Conf_Item[Test.Finshed_Count].toInt();
    } else {
        return;
    }
    ...
    if (Test.Finshed_Count == 0) {
        if ((Conf_Item[Test.Finshed_Count].toInt() == 3) || \
                (Conf_Item[Test.Finshed_Count].toInt() == 4)) {
            PLC_test_to_Switchover(Ciximinghua, 18, 2); // 断开交流接触器
            usleep(5000); usleep(5000); usleep(5000); usleep(5000); // 延时20ms
        } else {
            PLC_test_to_Switchover(Ciximinghua, Station_Chose, 2); // 吸合交流接触器
            usleep(5000); usleep(5000); usleep(5000); usleep(5000); // 延时20ms
        }
    } else {
        if ((Ciximinghua_Old == 3) || \
                (Ciximinghua_Old == 4)) {
            if ((Conf_Item[Test.Finshed_Count].toInt() == 3) || \
                    (Conf_Item[Test.Finshed_Count].toInt() == 4)) {
                // 无
                ...
                PLC_test_to_Switchover(Ciximinghua, Station_Chose, 2); // 吸合交流接触器
                usleep(5000); usleep(5000); usleep(5000); usleep(5000);
            }
        } else {
            if ((Conf_Item[Test.Finshed_Count].toInt() == 3) || \
                    (Conf_Item[Test.Finshed_Count].toInt() == 4)) {
    ...
    Ciximinghua_Old = Conf_Item[Test.Finshed_Count].toInt();
}
...
void w_Test::Test_Start()
{
    ...
    case 1:
        qDebug() << "w_Test::Test_Start() Test RES" << DCR_Total_Number;
        DCR_Test_Record_Number = 0;
        ...
        break;
    case 2:
        qDebug() << "w_Test::Test_Start() Test OPP" << MAG_Total_Number;
        MAG_Test_Record_Number = 0;
        TaskMagDirnumber_ME = 2;
        ME_Count = 0;
        TaskMagDirnumber = 5;
        TaskMagDirnumber_YY = 5;
        Test.Next_Finsh_Count++;
        Can_Test_MAG_Parameter();
        Can_Start_MAG();
        return;
        break;
    case 3:
        qDebug() << "w_Test::Test_Start() Test INS" << 1;
        TestIRACW = TestIW_IR;
        IRFinsh = false;
        Can_Start_IR(1);
        ...
        break;
    case 4:
        qDebug() << "w_Test::Test_Start() Test ACV" << 1;
        ...
        break;
    case 5:
        qDebug() << "w_Test::Test_Start() Test DCV";
        TestIRACW = TestIW_DCW;
        Can_Start_DCW();
        if (Test.Next_Finsh_Count != Test.Finshed_Count) { // 进行相间时的处理
            Test.Next_Finsh_Count--;
        }
        ...
        break;
    case 6:
        qDebug() << "w_Test::Test_Start() Test ITT" << IMP_Total_Number;
        if (Station_Chose == 0x13) {
            PLC_test_to_Switchover(BDVacuum, 1, 9); //  ((1 << 6)*256)
        } else {
            PLC_test_to_Switchover(BDVacuum, 2, 9); // ((1 << 1)*65536)
        }
        ...
        break;
    case 7:
        qDebug() << "w_Test::Test_Start() Test PWR";
        ...
        break;
    case 8:
        qDebug() << "w_Test::Test_Start() Test INDL" << INDL_Total_Number;
        INDL_Test_Record_Number = 0;
        Test.Next_Finsh_Count++;
        Can_Start_INDL();
        return;
        break;
    case 9:
        qDebug() << "w_Test::Test_Start() Test BLOCK" << 1;
        Can_Start_BLOCK();
        break;
    case 10:
        qDebug() << "w_Test::Test_Start() Test LVS" << 1;
        Can_Start_LVS();
        break;
    case 11:
        Test.Next_Finsh_Count++;
...
void w_Test::Can_PG_Start_Check() {
    PG_Test_State = 0x00;
    ...
    qDebug() << "w_Test::Can_PG_Start_Check 2018-11-05";
}
...
void w_Test::ChangeLable(int Lable, int number)
{
    ...
    if (WavePoint_strList[number][0].toInt() == 1) {
        ...
        list_Test << "123" << "124" << "125" << "126" << "127" << "128";
        str += list_Test.at(WavePoint_strList[number][1].toInt());
...
void w_Test::Test_Process() {
    ...
    Sigtest_Usart_Send_DeChang(QString("70  1 ")); // 进入忙状态
    Error_Mount = 0;
    ...
    PLC_test_to_Switchover(ZhongTe, (1<<7), 1);
    PLC_test_to_Switchover(ME_Hall, (1<<7), 16);
    PLC_test_to_Switchover(SAIPU, 0, 1);
    PLC_test_to_Switchover(Vacuum_Cooperate, (1<<7), 1);
    DC_IO_Put = (DC_IO_Put + (1<<7));
    PLC_test_to_Switchover(DeChang, (1<<7), 16);
    Test_Count++;
    Histogram_List.at(0)->setText(QString::number(Test_Count));
    for (i = 0; i < 14; i++) {
        Error_Item_Count[i] = 0;
    ...
    if ((Ini_IO_Model == Noise) && (Test_noise_enabled) && (Noise_CW_Time != 0)) { //
        PLC_test_to_Switchover(Noise, Station_Chose - 0x13 + 1 + 8, 7); // 切换进入反向测试
        Noise_step = 0;
        Noise_Stop->setSingleShot(TRUE);
...
{
    qDebug() << "w_Test::join_test_start Model Station" << Model << Station;
    if ((!(Test_GND_Volt/1000)) && (Ini_NineBoard == 2)) {
        ...
        Test_Net_WMessage(QString("Gnd Volt>20V %1").arg(Test_GND_Volt%1000));
        return;
    } else {
        lable_GND_Volt->hide();
    }
    ...
    if (Test_Curtain) {
        ...
        Prompt_Up_label->setAlignment(Qt::AlignCenter);
        Prompt_Up_label->setText(tr("光幕被遮挡\n停止启动"));
        Test_Net_WMessage("Light curtain Stop");
        Prompt_Widget->show();
        return;
    }
    ...
    if (AMP_Conneted) {
        ...
        Prompt_Up_label->setAlignment(Qt::AlignCenter);
        Prompt_Up_label->setText(tr("Inter Lock\n断开\n停止启动"));
        Test_Net_WMessage("Inter Lock---Stop");
        Prompt_Widget->show();
        return;
    }
    ...
    if (Test.Status_Ing == Test_Station_Ask) {
        ...
        if (((Ini_Stagnate == 0) && ((START_Model == 2) \
                                     || (START_Model == 1)))) { // -在滑罩模式下,取消防呆
        ...
        if (Ini_IO_Test) { // 启动信号启动
            ...
            if (Ini_IO_Model == LvZhi) { //
                Test.Status_Ing = Test_Station_Busy;
                PLC_test_to_Switchover(LvZhi, Station_Chose, 2);

                frame.can_id  = 0x2D;  frame.can_dlc = 0x03; // 绿智 初始气缸复位
                frame.data[0] = 0xf1;
                frame.data[1] = Station_Chose - 18;
                frame.data[2] = 0x00;
                canSend(frame, 88);

            } else if (Ini_IO_Model == BePuDa) { //
                PLC_test_to_Switchover(BePuDa, Station_Chose, 12);
            } else if (Ini_IO_Model == DeChang) { //
                DC_IO_Put = 3;
                PLC_test_to_Switchover(DeChang, DC_IO_Put, 16);
            } else {
                //
            }
        } else {
        ...
        if (ui->label_6->text() != tr("NG")) {
            ...
            Test_Net_WMessage("Start Model Error");
        }
    }
    qDebug() << "111";
}

void w_Test::Dechang_Connect(int value) {
    if (value == 8) {
        W_Connext = false;
    } else {
        //
...
bool w_Test::parameter_Issue_ing() {
    ...
    if (DLR_Issue_Ing) {
        ...
        return false;
    } else if (PWR_Issue_Ing)   {
        PWR_Issue_Ing = false;
        join_updatedata(QStringList(""));
        qDebug() << "Test 2017-12-20------> PWR_Issue_Ing Stop Update";
        return false;
    } else if (HV_Issue_Ing) {
        HV_Issue_Ing = false;
        for (i = 0; i < 1; i++) {
            join_updatedata(QStringList(""));
        }
        qDebug() << "Test 2017-12-20------> HV_Issue_Ing Stop Update";
        return false;
    }  else {
        //
    }
...
{
    qDebug() << "w_Test::join_test_occur_unqualified_Continue" << "--Stop--" << \
    ...
    case 2: // 滑罩 脚踏
        ...
        break;
    case 3:
        qDebug() << "w_Test::Continue NG STOP";
        if ((Test.Finshed_Count < (Conf_Item.size())) && \
                (Test.Status_Ing == Test_Station_Busy)) {
            Can_Stop(Back_board_ID[Conf_Item[Test.Finshed_Count].toInt()]);
            PWR_Par_Widget->hide();
            ...
            qDebug() << "w_Test::Continue Test All Ok";
        }
        if (Test_Judge_TotalResult()) { // 判定最终的显示状态  （显示  蜂鸣器 LED）
            Test_Beep_State(Beep.NG_Time, "NG");
            Sigtest_Usart_Send(QString("LEDR").toLatin1().data());
...
{
    qDebug() << "w_Test::join_test_occur_unqualified_Stop" << "--Stop--" << Stop \
    ...
    case 2: // 滑罩 脚踏
        if ((Test.Finshed_Count < (Conf_Item.size())) && \
            ...
            Can_Stop(0x22); Can_Stop(0x23); Can_Stop(0x24); Can_Stop(0x27);  Can_Stop(0x29);
            Error_Mount = 0;
            qDebug() << "w_Test::Stop" << ui->label_6->text() << Test.Status_Ing;
            if ((Test.Status_Ing == Test_Station_Inte) || \
                    (ui->label_6->text() != "NG")) {
...
{
    qDebug() << "w_Test::join_test_occur_unqualified_Ask()" << "--Stop--" << Stop \
             << "test state" << Test.Status_Ing;
    if (Conf_Item.size() == 0) {
        return;
    }
...
void w_Test::Test_Judge_State(QString Show_State)
{
    if (Show_State == "OK") { // Italic 倾斜
        ...
        ui->frame_state->\
                setStyleSheet("border:1px groove ;background-color: gold;"\
                              "border-radius:8px;border-color: #447684;");
        ui->label_station->setStyleSheet("color: rgb(255, 0, 0);border: none;"\
                                         "font: Bold 40pt Ubuntu  ;");
    } else {
        ui->frame_state->\
                setStyleSheet("border:1px groove ;background-color: #191919;"\
                              "border-radius:8px;border-color: #447684;");
    }
...
void w_Test::Test_Beep_State(double time_beep, QString Dis)
{
    ...
    for (i = 0; i < strTest.size(); i++) {
        ...
        for (int t = 0; t < Test_Item.size(); t++) {
            if (strTest.at(i).contains(Test_Item.at(t), Qt::CaseInsensitive)) {
                k = t;
                break;
            } else {
                //
...
//        }
    ...
    if (Ini_IO_Model == SAIPU) {
        if (Station_Chose == 0x13) {
            for (i = 0; i < strTest.size(); i++) {
                ...
                qDebug() << strTest.at(i);
                if (strTest.at(i).contains("1")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 0);
                } else if (strTest.at(i).contains("3")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 3);
                } else if (strTest.at(i).contains("5")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 1);
                } else if (strTest.at(i).contains("7")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 4);
                } else {
                    //
                }
            }
            ...
            for (i = 0; i < strTest.size(); i++) {
                ...
                qDebug() << strTest.at(i);
                if (strTest.at(i).contains("1")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 1)*256;
                } else if (strTest.at(i).contains("3")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 6);
                } else if (strTest.at(i).contains("5")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 5);
                } else if (strTest.at(i).contains("7")) {
                    IO_put_SaiPu = IO_put_SaiPu | (1 << 2);
                } else {
                    //
                }
            }
    ...
    PLC_test_to_Switchover(NanXin, 18, 5);
    PLC_test_to_Switchover(SAIPU, IO_put_SaiPu, 1);
    PLC_test_to_Switchover(NANXIN_Vacuum, 18, 17);

    int judge = 0;
    if (Dis == "NG") {
        judge = 0;
        qDebug() << "w_Test::Test_Beep_State NG -- CMD_In_B0";
        WriteMessage((6000), CMD_In_B0, QString("NG").toUtf8());
        //  PLC_test_to_Switchover(Bosch, 0x00, 7);
    } else {
        judge = 1;
        qDebug() << "w_Test::Test_Beep_State OK -- CMD_In_B0";
        WriteMessage((6000), CMD_In_B0, QString("OK").toUtf8());
        PLC_test_to_Switchover(Bosch, 0x00, 7);
    }
    QStringList Get_Set;
    if (Station_Chose == 0x13) {
    ...
    if (Dis == "NG") {
        ...
        Get_Set.append("NG");
        Histogram_List.at(2)->setText(QString::number(++Unqualified));
        PLC_test_to_Switchover(DeChang, ((1<<0)*256), 16);
        // -不合格时设置不合格的显示文本
    } else {
        IO_put = IO_put | (1<<6);
        PLC_test_to_Switchover(NanXin, Station_Chose, 3);
        PLC_test_to_Switchover(KaiYu, 0x00, 6);
        Get_Set.append("OK");
        Histogram_List.at(1)->setText(QString::number(++Qualified));
        PLC_test_to_Switchover(DeChang, (1<<6), 16);
        // -合格时设置合格的显示文本
    }
    ...
    qDebug() << "w_Test::Test_Beep_State   IO_put" << IO_put;
    PLC_test_to_Switchover(ZhongTe, IO_put, 1);
    PLC_test_to_Switchover(ME_Hall, IO_put, 16);
    PLC_test_to_Switchover(Vacuum_Cooperate, IO_put, 1);
    ...
    if (Ini_IO_Model == DeChang) {
        if (Back_ACW_Result) {
            Back_ACW_Result = false;
            Sigtest_Usart_Send_DeChang(QString("67  7 "));
            Test_Data = ACW_Result_DeChang;
            for (i = Test_Data.size(); i < 10; i++) {
                Test_Data = Test_Data.append(" ");
            }
            Sigtest_Usart_Send_DeChang(Test_Data);
        } else {
            //
        }
        Sigtest_Usart_Send_DeChang(QString("70  2 ")); // 进入待机状态
    } else {
        //
    }
}
...
void w_Test::Can_Start_DLR()
{
    ...
    if (Connect_Test) {
        if (NetTest_Ready_DLR) {
            ...
            qDebug() << "w_Test::Can_Start_DLR() Wait DLR Ready";
            return;
        }
    } else {
        //
...
void w_Test::Can_Test_MAG_Parameter() {
    int i;
    for (i = 0; i < 8; i++) {
        if (strOPP.at(NUM_OPP*i+3).toInt() == 2) {
            frame.can_id = 0x22; frame.can_dlc = 0x05;
            frame.data[0] = 0x04;
            frame.data[1] = i;
            frame.data[2] = strOPP.at(i*NUM_OPP).toInt();
            frame.data[3] = strOPP.at(i*NUM_OPP + 1).toInt();
            if (Station_Chose == 0x13) {
                frame.data[4] = strOPP.at(i*NUM_OPP + 5).toInt();
            } else if(Station_Chose == 0x14)  {
                if (strOPP.at(i*NUM_OPP + 10).toInt() == 0) {
                    frame.data[4] = strOPP.at(i*NUM_OPP + 5).toInt();
                } else {
                    frame.data[4] = strOPP.at(i*NUM_OPP + 10).toInt();
                }
            } else {
                frame.data[4] = strOPP.at(i*NUM_OPP + 5).toInt();
            }
...
void w_Test::Test_New_Hall() {
    qDebug() << "Test_New_Hall()";
    double stropp_at_one_double;
    double stropp_at_two_double;
    int    stropp_at_one_int;
    int    stropp_at_two_int;
    stropp_at_one_double = 0;
    stropp_at_two_double = 0;
    stropp_at_one_int = 0;
    stropp_at_two_int = 0;
    frame.can_id = 0x25;  frame.can_dlc = 0x07;
    frame.data[0] = 0x04;
    frame.data[1] = 0x00;
    frame.data[2] = 0x00;
    frame.data[3] = 0x00;
    ...
    stropp_at_one_double = strOPP.at(6561).toDouble()*100;
    stropp_at_two_double = strOPP.at(6564).toDouble()*1000;
    stropp_at_one_int = stropp_at_one_double;
    stropp_at_two_int = stropp_at_two_double;

    frame.can_id = 0x25;  frame.can_dlc = 0x06;
    frame.data[0] = 0x01;
    frame.data[1] = stropp_at_one_int/256;
    frame.data[2] = stropp_at_one_int%256;
    frame.data[3] = stropp_at_two_int/256;
    frame.data[4] = stropp_at_two_int%256;
    frame.data[5] = Station_Chose;
    canSend(frame, wTest_Surface);
    usleep(500);
}
...
void w_Test::Can_Start_MAG()
{
    ...
    if (Connect_Test) {
        if (NetTest_Ready_MAG) {
            ...
            qDebug() << "w_Test::Can_Start_MAG() Wait MAG Ready";
            return;
        }
    } else {
        //
    }
    qDebug() << "w_Test::Can_Start_MAG()" << Ini_MAG_Device << MAG_Drive;
    if (Ini_MAG_Device) {
        if (!MAG_Drive) {
            Test_MAG_Ready = true;
...
void w_Test::Test_WL_Hall_FInsh() {
    int i;
    i = 0;
    if (Ini_IO_Model == WL_Hall) {
        Sigtest_Usart_Send_WEILING("01100A0700020400000000CCE9"); // 电流设置
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        Sigtest_Usart_Send_WEILING("01100A0000010200028D91"); // 使能电流
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        Sigtest_Usart_Send_WEILING("01100A05000204000000004D30"); // 电压设置
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        Sigtest_Usart_Send_WEILING("01100A000001020001CD90"); // 使能电压
        PLC_test_to_Switchover(WL_Hall, 0, 7);
    } else if (Ini_IO_Model == ME_Hall) {
        Sigtest_Usart_Send_WEILING("01100A0700020400000000CCE9"); // 电流设置
        for (i = 0; i < 10; i++) {
            usleep(10000);
...
quint16 w_Test::crc16(QByteArray hex) {
    quint16 crc = 0xFFFF;
...
uint16_t w_Test::crc16_modbus(char *data, uint16_t length)
{
    uint8_t i;
...
void w_Test::Test_WL_Hall() {
    int i;
    i = 0;
    uint16_t crc;
    crc = 0;
    QByteArray QByteArray_data;
    QString volt_string;
    ...
    vlot.Result = strOPP.at(6561).toDouble();
    current.Result = strOPP.at(6562).toDouble();

    if (Ini_IO_Model == WL_Hall) {
        // 吸合交流接触器
        PLC_test_to_Switchover(WL_Hall, Station_Chose - 0x13 + 1, 7);
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        /* 打开电源 */
        ...
        volt_string = "";
        volt_string = volt_string + QString("%1").arg(vlot.dat[3]&0xFF, 2, 16, QLatin1Char('0'));
        volt_string = volt_string + QString("%1").arg(vlot.dat[2]&0xFF, 2, 16, QLatin1Char('0'));
        volt_string = volt_string + QString("%1").arg(vlot.dat[1]&0xFF, 2, 16, QLatin1Char('0'));
        volt_string = volt_string + QString("%1").arg(vlot.dat[0]&0xFF, 2, 16, QLatin1Char('0'));
        volt_string = "01100A05000204" +  volt_string;  ;
        QByteArray_data.clear();  QByteArray_data.append(volt_string);
        crc = crc16(QByteArray::fromHex(QByteArray_data));
        volt_string = volt_string + QString("%1").arg((crc%256)&0xFF, 2, 16, QLatin1Char('0'));
        volt_string = volt_string + QString("%1").arg((crc/256)&0xFF, 2, 16, QLatin1Char('0'));
        Sigtest_Usart_Send_WEILING(volt_string); // 电压设置
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        Sigtest_Usart_Send_WEILING("01100A000001020001CD90"); // 使能电压
        ...
        current_string = "";
        current_string = current_string + QString("%1").arg(current.dat[3]&0xFF, 2, 16, QLatin1Char('0'));
        current_string = current_string + QString("%1").arg(current.dat[2]&0xFF, 2, 16, QLatin1Char('0'));
        current_string = current_string + QString("%1").arg(current.dat[1]&0xFF, 2, 16, QLatin1Char('0'));
        current_string = current_string + QString("%1").arg(current.dat[0]&0xFF, 2, 16, QLatin1Char('0'));
        current_string = "01100A07000204" +  current_string;
        QByteArray_data.clear();  QByteArray_data.append(current_string);
        crc = crc16(QByteArray::fromHex(QByteArray_data));
        current_string = current_string + QString("%1").arg((crc%256)&0xFF, 2, 16, QLatin1Char('0'));
        current_string = current_string + QString("%1").arg((crc/256)&0xFF, 2, 16, QLatin1Char('0'));
        Sigtest_Usart_Send_WEILING(current_string); // 电流设置
        for (i = 0; i < 10; i++) {
            usleep(10000);
        }
        Sigtest_Usart_Send_WEILING("01100A0000010200028D91"); // 使能电流
        ...
        TaskMagDirnumber = 2;
        WL_Back_Count = 0;
        WL_Start->start(100); // 启动定时器,记录定时器启动计时// 打开定时器,读取IO 10 次数;
        return;
    } if (Ini_IO_Model == ME_Hall) {
        // 吸合交流接触器
        if (ME_Count) {
            TaskMagDirnumber_ME = 2;
            PLC_test_to_Switchover(ME_Hall, ((1<<2)*65536 | (1<<7)), 16);
        } else {
            TaskMagDirnumber = 2;
            PLC_test_to_Switchover(ME_Hall, (((1<<6)*256) | ((1<<7))), 16);
        }
...
/******************************************************************************
  * brief:      启动绝缘进行测试
******************************************************************************/
void w_Test::Can_Start_IR(int continue_Test)
{
    IR_Issue_Ing = true;
    frame.can_id  = 0x2C;  frame.can_dlc = 0x02; // 高压回路
    frame.data[0] = 0x12; // 高压回路
    ...
    if (Connect_Test) {
        if (NetTest_Ready_IR) {
            ...
            if (continue_Test == 100) {
                //
            } else {
                qDebug() << "w_Test::Can_Start_IR() Wait IR Ready";
                return;
            }
        }
    } else {
...
//    Sigtest_Save_Data(QStringList(""), QString(""), 2);
    if (Ini_ACWandIR) { // -相间绝缘
        ...
        if (IR_First_finsh) {
            IR_First_Test_Enabled = 2;
            frame.data[4] = 0x0B;           // 测试组
        } else {
            IR_First_Test_Enabled = 1;
...
/******************************************************************************
  * brief:      启动耐压进行测试
******************************************************************************/
void w_Test::Can_Start_ACW(int continue_Test)
{
    ACW_Issue_Ing = true;
    frame.can_id  = 0x2C;  frame.can_dlc = 0x02; // 高压回路
    frame.data[0] = 0x12; // 高压回路
    ...
    if (Connect_Test) {
        if (NetTest_Ready_ACW) {
            ...
            if (continue_Test == 100) {
                ...
                qDebug() << "w_Test::Can_Start_ACW() Wait ACW Ready";
                return;
            }
        }
    } else {
    ...
    if (ACW_First_finsh) { // 2
        if (Test_Model_Exhaust_ACW) {
            if (Vacuum_Ready) {
                Test_ACW_Ready = true;
                qDebug() << "Can_Start_ACW---2 return";
                return;
            } else {
                //
            }
...
//    Sigtest_Save_Data(QStringList(""), QString(""), 3);
    if (Ini_ACWandIR) { // 相间耐压
        ...
        frame.data[4] = 0x00;
        if (ACW_First_finsh) {
            ACW_First_Test_Enabled = 2;
            frame.data[4] = 0x0A;           // 测试组
        } else {
            ACW_First_Test_Enabled = 1;
            frame.data[4] = 0x00;           // 测试组
        }
        canSend(frame, wTest_Surface);
        usleep(500);
    }
...
void w_Test::Can_Start_HV()
{
    ...
    if (Connect_Test) {
        if (NetTest_Resdy_HV) {
            ...
            qDebug() << "w_Test::Can_Start_HV() Wait HV Ready";
            return;
        }
    } else {
        //
...
void w_Test::Can_Start_DCW()
{
    ...
    usleep(500);
    if (Connect_Test) {
        if (NetTest_Ready_DCW) {
            //
        } else {
            qDebug() << "w_Test::Can_Start_DCW() Wait DCW Ready";
            return;
        }
    } else {
        //
    }
    qDebug() << "w_Test::Can_Start_DCW()";

    NetTest_Ready_DCW = false;
    if (DCW_Test_Count < 0) {
        DCW_Test_Count = DCW_Test_Total-1;
    }
    Sigtest_Save_Data(QStringList(""), QString(""), 4);
    if (Ini_ACWandIR) { //
        frame.can_id = 0x23;
        frame.can_dlc = 0x05;
        frame.data[0] = 0x01;
        ...
        frame.data[2] = 0x00;
        frame.data[3] = Station_Chose;
        frame.data[4] = DCWgroup_num.at(DCW_Test_Count).toInt();
        canSend(frame, wTest_Surface);
        usleep(500);
    } else {
        frame.can_id = 0x23;
...
{
    if ((Ini_IMP_36) && (strITT.at(13).toInt() == 2)) {
        PLC_test_to_Switchover(BaoJiaLiYa, Station_Chose, 2);
        usleep(10000); usleep(10000); usleep(10000);
    } else {
        //
    }
    ...
    if (Connect_Test) {
        if (NetTest_Ready_IMP) {
            ...
            qDebug() << "w_Test::Can_Start_IMP() Wait IMP Ready";
            return;
        }
    } else {
        //
    ...
    if (Test_Model_Vacuum) {
        qDebug() << "Test_Model_Vacuum" << Test_Model_Vacuum << Vacuum_Ready;
        if (!Vacuum_Ready) {
            Test_IMP_Ready = true;
            return;
        } else {
...
void w_Test::Can_Start_PWR(int str)
{
    ...
    if (Ini_Set_pwr_Turn) {
        if (PWR_SetData.at(1 + PWR_Test_Count).toInt()%2) {
            PLC_test_to_Switchover(MaiXing, 100 + Station_Chose, 13);
        } else {
            PLC_test_to_Switchover(MaiXing, Station_Chose, 13);
        }
    } else {
        //
    }
    ...
    PG_list.clear();
    Fre_Ng = true;
    PLC_test_to_Switchover(NanXin, Station_Chose, 5);
    PLC_test_to_Switchover(NANXIN_Vacuum, Station_Chose, 17);
    ...
    if (Connect_Test) {
        if (NetTest_Resdy_PWR) {
            //
        } else {
            qDebug() << "w_Test::Can_Start_PWR Wait PWR Ready";
            return;
        }
    } else {
        //
    ...
    if (str  ==  1) {
        ...
        for (i = 0; i < strPWR.at(33).toInt(); i++) {
            usleep(1000);
            qApp->processEvents();
        }
        // -不进行任何操作
    ...
    qDebug() << "w_Test::Can_Start_PWR" << PWR_Test_Data.size() << PWR_Test_Data << PWR_Test_Count;
    ...
    if ((Ini_Motor == QString(tr("PG"))) && ((strPWR.at(5 + position_grade).toInt() == 2))) {
        ...
        canSend(frame, wTest_Surface);
        usleep(500);
        qDebug() << "w_Test::Can_Start_PWR Down PG";
    } else {
        //
    }
...
void w_Test::Can_Start_INDL()
{
    ...
    if (Connect_Test) {
        if (NetTest_Ready_IND) {
            ...
            qDebug() << "w_Test::Can_Start_INDL() Wait IND Ready";
            return;
        }
    } else {
        //
...
void w_Test::Can_Start_BLOCK()
{
    ...
    if (Ini_Power_Chose == 0) {     //-无电源
        ...
        frame.data[3] = ((4 << 4) | ((pwr_volt + strBLOCK.at(208).toInt())/256));
        // 电压 (增加 电源 频率)
...
void w_Test::Can_Start_BLDCHALL() {
    ...
    if (Connect_Test) {
        if (NetTest_Resdy_HALL) {
            ...
            qDebug() << "w_Test::Can_Start_BLDCHALL() Wait Hall Ready";
            return;
        }
    } else {
        //
...
void w_Test::Can_Start_LOAD() {
    double strLOAD_at_one_double;
    double strLOAD_at_two_double;
    int strLOAD_at_one_int;
    int strLOAD_at_two_int;

    strLOAD_at_one_double = 0;
    strLOAD_at_two_double = 0;
    strLOAD_at_one_int = 0;
    strLOAD_at_two_int = 0;
    ...
    if (Connect_Test) {
        if (NetTest_Resdy_LOAD) {
            ...
            qDebug() << "w_Test::Can_Start_LOAD() Wait Load Ready";
            return;
        }
    } else {
        //
    ...
    strLOAD_at_one_double = strLOAD.at(1).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*2450 + 5) / 10;
    strLOAD_at_two_double = strLOAD.at(110).toDouble();
    strLOAD_at_two_int = (strLOAD_at_two_double*10000 + 5) / 10;
    ...
    frame.data[4] = (strLOAD_at_one_int - 6)%256; //
    frame.data[5] = (strLOAD_at_one_int - 6)/256; //
    frame.data[6] = (strLOAD_at_two_int)%256;
    frame.data[7] = (strLOAD_at_two_int)/256;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    }
    ...
    strLOAD_at_one_double = strLOAD.at(1).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*2450 + 5) / 10;
    strLOAD_at_two_double = strLOAD.at(119).toDouble();
    strLOAD_at_two_int = (strLOAD_at_two_double*10000 + 5) / 10;
    ...
    frame.data[4] = (strLOAD_at_one_int - 6)%256; // *4095/16.8
    frame.data[5] = (strLOAD_at_one_int - 6)/256; // *4095/16.8
    frame.data[6] = (strLOAD_at_two_int) % 256;
    frame.data[7] = (strLOAD_at_two_int) / 256;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) {  // 下发参数延时等待 10ms
    ...
    strLOAD_at_one_double = strLOAD.at(2).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*2450 + 5) / 10;
    strLOAD_at_two_double = strLOAD.at(114).toDouble();
    strLOAD_at_two_int = (strLOAD_at_two_double*10000 + 5) / 10;
    ...
    strLOAD_at_one_double = strLOAD.at(2).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*2450 + 5) / 10;
    strLOAD_at_two_double = strLOAD.at(115).toDouble();
    strLOAD_at_two_int = (strLOAD_at_two_double*10000 + 5) / 10;
    ...
    frame.data[4] = (strLOAD_at_one_int - 6)%256; // *4095/16.8
    frame.data[5] = (strLOAD_at_one_int - 6)/256; // *4095/16.8
    frame.data[6] = (strLOAD_at_two_int*1000)%256;
    frame.data[7] = (strLOAD_at_two_int*1000)/256;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    }

    strLOAD_at_one_double = strLOAD.at(0).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*10 + 5) / 10;
    strLOAD_at_two_int = (strLOAD_at_one_double*1000 + 5) / 10;
    ...
    if (strLOAD.at(5).toInt() == 1) { // 01 菊水
        ...
        frame.data[4] = (strLOAD_at_one_int)%256;
        frame.data[5] = (strLOAD_at_one_int)/256;
        frame.data[6] = 0x00;
        frame.data[7] = 0x00;
    } else { // 02 天虹
        frame.data[3] = strLOAD.at(5).toInt();
        frame.data[4] = (strLOAD_at_two_int)/1000;
        frame.data[5] = (strLOAD_at_two_int)%1000/100;
        frame.data[6] = (strLOAD_at_two_int)%100/10;
        frame.data[7] = (strLOAD_at_two_int)%10;
    }
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    ...
    strLOAD_at_one_double = strLOAD.at(0).toDouble();
    strLOAD_at_one_int = (strLOAD_at_one_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x05; // 启动测试
    frame.data[0] = 0x01;
    frame.data[1] = Station_Chose - 0x12;
    frame.data[2] = strLOAD.at(14).toInt(); // 测试 HALL-A
    frame.data[3] = (strLOAD_at_one_int)%256;
    frame.data[4] = (strLOAD_at_one_int)/256;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    ...
    PG_Start_count = 2;
    PG_Start_Start->setSingleShot(TRUE);
    PG_Start_Start->start(800);
}

void w_Test::Can_Start_NOLOAD() {
    double strNOLOAD_at_one_double;
    double strNOLOAD_at_two_double;
    int strNOLOAD_at_one_int;
    int strNOLOAD_at_two_int;

    strNOLOAD_at_one_double = 0;
    strNOLOAD_at_two_double = 0;
    strNOLOAD_at_one_int = 0;
    strNOLOAD_at_two_int = 0;

    NOLOAD_Issue_Ing = true;
    int j = 0;
    if (Ini_IO_Model == Bosch) { //
        if (Station_Chose == 0x13) {
            PLC_test_to_Switchover(Bosch, 0x03, 7);
        } else if (Station_Chose == 0x14) {
            PLC_test_to_Switchover(Bosch, 0x05, 7);
        }
    }
    ...
    if (Connect_Test) {
        if (NetTest_Resdy_NOLOAD) {
            ...
            qDebug() << "w_Test::Can_Start_NOLOAD() Wait NOLoad Ready";
            return;
        }
    } else {
        //
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(1).toDouble();
    strNOLOAD_at_two_double = strNOLOAD.at(110).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*2450 + 5) / 10;
    strNOLOAD_at_two_int = (strNOLOAD_at_two_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x08; // 下发vcc 电压 缓升时间
    frame.data[0] = 0x03;
    frame.data[1] = Station_Chose - 0x12;
    frame.data[2] = 0x01; // 01 vcc 02 vsp 03 vm
    frame.data[3] = 0x01; // 01 缓升 02 缓降
    frame.data[4] = (strNOLOAD_at_one_int-6)%256; // *4095/16.8
    frame.data[5] = (strNOLOAD_at_one_int-6)/256; // *4095/16.8
    frame.data[6] = (strNOLOAD_at_two_int)%256;
    frame.data[7] = (strNOLOAD_at_two_int)/256;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    }
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(1).toDouble();
    strNOLOAD_at_two_double = strNOLOAD.at(119).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*2450 + 5) / 10;
    strNOLOAD_at_two_int = (strNOLOAD_at_two_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x08; // 下发vcc 电压 缓降时间
    frame.data[0] = 0x03;
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(2).toDouble();
    strNOLOAD_at_two_double = strNOLOAD.at(114).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*2450 + 5) / 10;
    strNOLOAD_at_two_int = (strNOLOAD_at_two_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x08; // 下发vsp 电压 缓升时间
    frame.data[0] = 0x03;
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(2).toDouble();
    strNOLOAD_at_two_double = strNOLOAD.at(115).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*2450 + 5) / 10;
    strNOLOAD_at_two_int = (strNOLOAD_at_two_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x08; // 下发vsp 电压 缓降时间
    frame.data[0] = 0x03;
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(0).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*10 + 5) / 10;
    strNOLOAD_at_two_int = (strNOLOAD_at_one_double*1000 + 5) / 10;
    frame.can_id  = 0x2C;  frame.can_dlc = 0x08; // 下发vm 缓升时间
    frame.data[0] = 0x03;
    frame.data[1] = Station_Chose - 0x12;
    frame.data[2] = 0x03; // 01 vcc    02 vsp    03 vm
    if (strNOLOAD.at(5).toInt() == 1) { // 01 菊水
        ...
        frame.data[4] = (strNOLOAD_at_one_int)%256;
        frame.data[5] = (strNOLOAD_at_one_int)/256;
        frame.data[6] = 0x00;
        frame.data[7] = 0x00;
    } else if (strNOLOAD.at(5).toInt() == 2) { // 02 天虹
        frame.data[3] = strNOLOAD.at(5).toInt();
        frame.data[4] = (strNOLOAD_at_two_int)/1000;
        frame.data[5] = (strNOLOAD_at_two_int)%1000/100;
        frame.data[6] = (strNOLOAD_at_two_int)%100/10;
        frame.data[7] = (strNOLOAD_at_two_int)%10;
    } else {
        frame.data[3] = strNOLOAD.at(5).toInt();
        frame.data[4] = (strNOLOAD_at_one_int)%256;
        frame.data[5] = (strNOLOAD_at_one_int)/256;
    ...
    strNOLOAD_at_one_double = strNOLOAD.at(3).toDouble();
    strNOLOAD_at_one_int = (strNOLOAD_at_one_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x05; // 启动测试
    frame.data[0] = 0x01;
    frame.data[1] = Station_Chose - 0x12;
    frame.data[2] = 0x02; // 测试 HALL-A
    frame.data[3] = (strNOLOAD_at_one_int) % 256;
    frame.data[4] = (strNOLOAD_at_one_int) / 256;
    canSend(frame, wTest_Surface);

    PG_Start_count = 1;
    PG_Start_Start->setSingleShot(TRUE);
    PG_Start_Start->start(1000);
    NOLOAD_Issue_Ing = false;
}

void w_Test::PG_State_timeout() {
    qDebug() << "PG_State_timeout()";
    if (PG_Start_count == 1) {
...
void w_Test::Can_Start_BEMF() {
    BEMF_Issue_Ing = true;
    int j;
    double strBEMF_at_six_double;
    double strBEMF_at_four_double;
    int strBEMF_at_six_int;
    int strBEMF_at_four_int;

    j = 0;
    strBEMF_at_six_double = 0.0;
    strBEMF_at_four_double = 0.0;
    strBEMF_at_six_int = 0;
    strBEMF_at_four_int = 0;

    if (Connect_Test) {
        if (NetTest_Resdy_BEMF) {
            //
        } else {
            qDebug() << "w_Test::Can_Start_BEMF() Wait BEMF Ready";
            return;
        }
    } else {
        //
    ...
    NetTest_Resdy_BEMF = false;
    qDebug() << "w_Test::Can_Start_BEMF()";

    strBEMF_at_six_double = (strBEMF.at(6).toDouble()*10 + 5) / 10;
    strBEMF_at_six_int = strBEMF_at_six_double;
    ...
    frame.data[2] = 0x01; // 01 vcc 02 vsp 03 vm
    frame.data[3] = 0x01; // 01 缓升 02 缓降
    frame.data[4] = (strBEMF_at_six_int * 245 - 6) % 256; // *4095/16.8
    frame.data[5] = (strBEMF_at_six_int * 245 - 6) / 256; // *4095/16.8
    frame.data[6] = 1;
    frame.data[7] = 0;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
    ...
    strBEMF_at_four_double = strBEMF.at(4).toDouble();
    strBEMF_at_four_int = (strBEMF_at_four_double*10000 + 5) / 10;
    frame.can_id = 0x2C;  frame.can_dlc = 0x05;  //启动三角形
    frame.data[0] = 0x01; // 启动测试
    frame.data[1] = Station_Chose - 0x12;
    frame.data[2] = 0xfe; //
    frame.data[3] = strBEMF_at_four_int % 256;
    frame.data[4] = strBEMF_at_four_int / 256;
    canSend(frame, wTest_Surface);
...
void w_Test::Test_Net_WMessage(QString data) {
    if (START_Model == 3) {
...
void w_Test::Test_WMessage(int number, QString Waring_Text)
{
    if (START_Model == 3) {
        ...
        message->addButton(QMessageBox::Ok)->\
                setStyleSheet("height:39px;width:75px;border:5px groove;border:none;"
                              "border-radius:10px;padding:2px 4px;");
...
void w_Test::Test_PWR_Widget(double volt, int value)
{
    ...
    if (Conf_Item[Test.Finshed_Count].toInt() == 1) { // -测试假功率
        ...
        if ((False_Test) && (value == 2) && (volt < 5)) { // -反电动势大于5V
            ...
            if (BEMF_Count > 4) {
                BEMF_Count = 0;
                Turn_Elec_forceing = true;
                ui->label_5->setText(QString(Turn_Elec_force_List.join(" ")));
                join_test_stop(3);
                whom = QString::number(volt).toUtf8().data();
                sprintf(copystr, QString(tr("反电动势高\n%sV\nSTOP")).toUtf8().data(), whom);
                Prompt_Up_label->setAlignment(Qt::AlignCenter);
                Prompt_Up_label->setText(copystr);
                Test_Net_WMessage(QString("BEMF High %1").arg(volt));
                Prompt_Widget->show();
            } else {
                Can_Start_DCR_Beforce();
            }
...
void w_Test::Pub_Main_to_test(QStringList list, QString str, int value, int flag)
{
    ...
    case 9:
        if (value) {
            ...
            PLC_test_to_Switchover(ZhongTe, 0, 1);
            PLC_test_to_Switchover(SAIPU, 0, 1);
            PLC_test_to_Switchover(Vacuum_Cooperate, 0, 1);
            PLC_test_to_Switchover(Ciximinghua, 18, 2); // 断开交流接触器
            PLC_test_to_Switchover(MaiXing, 0X18, 13);
            PLC_test_to_Switchover(BaoJiaLiYa, 18, 2); // 断开交流接触器
            join_updateTime();
            PWR_Par_Widget->hide();
            Test.Status_Ing = Test_Station_Free;
        } else {
        ...
        break;
    case 10:
        qDebug() << "w_Test::Pub_Main_to_test 10 Net single item test" << str << Test.Status_Ing;
        if (Test.Status_Ing  ==  Test_Station_Busy) {
            //
        } else {
            qDebug() << " return---> w_Test::Pub_Main_to_test 10";
            return;
        }
        if (str == QString(tr("DCR"))) {
            ...
            NetTest_Ready_ACW = true;
            Can_Start_ACW(1);
        }  else if (str == QString(tr("DCW"))) {
            NetTest_Ready_DCW = true;
            Can_Start_DCW();
            ...
            Can_Start_BEMF();
        } else if (str == QString(tr("PWR"))) {
            NetTest_Resdy_PWR = true;
            Can_Start_PWR(1);
        } else if (str == QString(tr("PWR_1"))) {
            NetTest_Resdy_PWR = true;
            Can_Start_PWR(2);
        } else if (str == QString(tr("HV"))) {
            NetTest_Resdy_PWR = true;
            Can_Start_HV();
        } else {
            //
    ...
    case 12:
        if (Kongshi_Stall) {
            Kongshi_Stall = false;
            Pri_Test_Time_Stop(QString(""), 110);
        }
        qDebug() << "w_Test::Pub_Main_to_test 12" << Test.Status_Ing << NetTest_Ready_IMP;
        if ((Test.Status_Ing == Test_Station_Busy) && (Connect_Test)) {
            if (NetTest_Ready_DLR) {
                for (i = 0; i < DCR_Total_Number; i++) {
                    join_updatedata(QStringList(""));
                }
                qDebug() << "w_Test::Pub_Main_to_test 12 DLR Stop Update";
                return;
            } else if (NetTest_Ready_MAG) {
                for (i = 0; i < MAG_Total_Number; i++) {
                    join_updatedata(QStringList(""));
                }
                qDebug() << "w_Test::Pub_Main_to_test 12 MAG Stop Update";
                return;
                ...
                qDebug() << "w_Test::Pub_Main_to_test 12 IR Stop Update";
                return;
                ...
                qDebug() << "w_Test::Pub_Main_to_test 12 ACW Stop Update";
                return;
            } else if (NetTest_Ready_IMP) {
                for (i = 0; i < IMP_Total_Number; i++) {
                    join_updatedata(QStringList(""));
                }
                qDebug() << "w_Test::Pub_Main_to_test 12 IMP Stop Update";
                return;
            } else if (NetTest_Ready_IND) {
                for (i = 0; i < INDL_Total_Number; i++) {
                    join_updatedata(QStringList(""));
                }
                qDebug() << "w_Test::Pub_Main_to_test 12INDL Stop Update";
                return;
            } else if (NetTest_Resdy_HALL)   {
                join_updatedata(Hall_Update);
                qDebug() << "w_Test::Pub_Main_to_test 12 HALL Stop Update";
                return;
            } else if (NetTest_Resdy_LOAD)   {
                join_updatedata(QStringList(""));
                qDebug() << "w_Test::Pub_Main_to_test 12 LOAD Stop Update";
                return;
            } else if (NetTest_Resdy_NOLOAD) {
                join_updatedata(QStringList(""));
                qDebug() << "w_Test::Pub_Main_to_test 12 NOLOAD Stop Update";
                return;
            } else if (NetTest_Resdy_BEMF)   {
                join_updatedata(QStringList(""));
                qDebug() << "w_Test::Pub_Main_to_test 12 BEMF Stop Update";
                return;
            } else if (NetTest_Resdy_PWR)   {
                join_updatedata(QStringList(""));
                qDebug() << "w_Test::Pub_Main_to_test 12 PWR Stop Update";
                return;
            } else if (NetTest_Resdy_HV)   {
                join_updatedata(QStringList(""));
                qDebug() << "w_Test::Pub_Main_to_test 12 HV Stop Update";
                return;
            }  else {
                //
            }
        ...
        PLC_test_to_Switchover(SAIPU, 0, 1);
        PLC_test_to_Switchover(NanXin, 18, 5);
        PLC_test_to_Switchover(Noise, 0x00, 7);
        Noise_Stop->stop();
        ui->label_noise->hide();
        Noise_Blink->stop();
        Ask_Widget->hide();
    ...
    case 22:
        if (str == QString(tr("ABC"))) {
            Ini_ROT = true;
            break; // G201807036 可能存在問題
        } else if ((str == "YY") || (str == "Y-YY") || (str == "D-YY") \
                || (str == "YY-PT")|| (str == "YY-1"))  {
            Ini_ROT = true;
            YY_Model = true;
        } else {
    ...
    case 26:
        if (value/10) { // 耐压判定
            qDebug() << "w_Test::Pub_Main_to_test 26 ACW" << value%10;
            if (value%10 == 1) {
            ...
            qDebug() << "w_Test::Pub_Main_to_test 26 IMP" << value%10;
            if (value%10 == 1) {
        ...
        if ((Test_Model_Vacuum) || (Test_Model_Vacuum_ACW)) {
            qDebug() << "w_Test::Pub_Main_to_test 26 Vacuum";
            Signal_test_to_pic(QStringList(""), 1, 12); // 打开真空泵
        } else {
            qDebug() << "w_Test::Pub_Main_to_test 26 !Vacuum";
    ...
    case 27:
        ...
        break;
    case 28:
        emit WriteMessage(6000, CMD_Put_73, QString(list.join(" ")).toUtf8()); // 传输上位机
        join_updatedata(list);
        break;
    case 29:
        emit WriteMessage(6000, CMD_Put_73, QString(list.join(" ")).toUtf8());
        join_updatedata(list);
        break;
    case 30:
        Hall_Update = list;
    ...
    case 32:
        if (value) {
            ...
            Prompt_Up_label->setAlignment(Qt::AlignCenter);
            Prompt_Up_label->setText(tr("Connect\nError"));
            Test_Net_WMessage("Connect Error");
            Prompt_Widget->show();
        }
        break;
    case 33:
        if (value == 0) { //
            Test_Model_Exhaust_ACW = true;
        } else { //
            Test_Model_Exhaust_ACW = false;
        }
        qDebug() << "3333333333333333333333333333" << Test_Model_Exhaust_ACW;
        break;
    case 34:
        Ini_IO_Delay      = list.at(1).toInt();
        Ini_IO_Delay_Time = list.at(2).toInt();
        break;
    case 118:
    ...
    case 120:
        A3 = "00000000";
        A1 = QString::number((value / 256), 2);
        A2 = QString::number((value % 256), 2);
        IO_Input_Data->setText(A3.mid(0, 8 - A1.size()) + A1 + " "\
                               + A3.mid(0, 8 - A2.size()) + A2);
        break;
    case 138:
        IO_Input_Data->setText(str);
        break;
    case 121:
    ...
    case 123:
        ...
        break;
    case 125:
        ADC_L = list.at(0).toInt();
        ADC_R = list.at(1).toInt();
        if (value) {
            ADC0_L_Value = list.at(0).toInt();
            ADC0_R_Value = list.at(1).toInt();
            return;
        } else {
            if (Station_Chose == 0x13) {
                if (ME_Count) {
                    qDebug() << "RRRRRRRRRRRRR";
                    if (ADC_R < (ADC0_R_Value - 50)) {
                        if (mag_hall_rotation) {
                            TaskMagDirnumber_ME = 1;
                        } else {
                            TaskMagDirnumber_ME = 0;
                        }
                    } else if (ADC_R > (ADC0_R_Value + 50)) {
                        if (mag_hall_rotation) {
                            TaskMagDirnumber_ME = 0;
                        } else {
                            TaskMagDirnumber_ME = 1;
                    ...
                    if (ADC_L < (ADC0_L_Value - 50)) {
                        if (mag_hall_rotation) {
                            TaskMagDirnumber = 1;
                        } else {
                            TaskMagDirnumber = 0;
                        }
                    } else if (ADC_L > (ADC0_L_Value + 50)) {
                        if (mag_hall_rotation) {
                            TaskMagDirnumber = 0;
                        } else {
                            TaskMagDirnumber = 1;
...
void w_Test::PLC_test_to_Switchover(int Factory, int32_t value, int chose) {
    if (Ini_IO_Model != Factory) {
        qDebug() << "return---> Switchover_Factory != Factory";
        return;
    } else {
        Signal_test_to_Switchover(Factory, value, chose);
    }
}
/******************************************************************************
  * version:    1.0
...
//        }
    ...
    if (strITT.size() >= 6561) {
        if (strITT.at(15).toInt() == 0) {
            for (x = 0; x < 8; x++) {
                strITT.replace(x*NUM_ITT+15, strITT.at(x*NUM_ITT + 10));
                strITT.replace(x*NUM_ITT+14, strITT.at(x*NUM_ITT + 9));
            }
        }
    } else {
        //
...
void w_Test::Test_Overtime_Alarm(int Bug_Count)
{
    ...
    if ((Bug_Count) > 100) {
        Bug_Count = 0;
        qDebug() << "w_Test::Test_Overtime_Alarm";
        Test_Beep_State(Beep.NG_Time, "NG");
        Sigtest_Usart_Send(QString("LEDR").toLatin1().data());
        Test_Judge_State("NG");
        Test.Status_Ing = Test_Station_Free;
        ...
        if (False_Test) {
            False_Test = false;
            Prompt_Up_label->setAlignment(Qt::AlignCenter);
            Prompt_Up_label->setText(tr("反电动势\n无结果\nSTOP"));
            Test_Net_WMessage("BEMF no Result");
            Prompt_Widget->show();
            t = 100;
        } else {
            Prompt_Up_label->setAlignment(Qt::AlignCenter);
            Prompt_Up_label->setText(tr("%1板异常\n停止测试").arg\
                                  (Test_Item.at(Conf_Item.at(Test.Finshed_Count).toInt())));
            if (Ini_IO_Model == NEW_Hall) {
                if ((Conf_Item.at(Test.Finshed_Count).toInt()) == 2) {
                    Prompt_Up_label->setText(tr("B05板异常\n停止测试"));
                } else {
                    //
                }
            } else {
            ...
            Test_Net_WMessage(QString("Board %1 Stop").arg(Conf_Item.at(Test.Finshed_Count).toInt()));
            Prompt_Widget->show();
            t = ((Conf_Item.at(Test.Finshed_Count).toInt()));
        }

        Singal_Record_Error(101, QString::number(t));
    } else {
        //
    }
}

void w_Test::Pri_Test_Time_Stop(QString str, int bug_count)
{
    str.clear();
    if (Kongshi_Stall) {
        Kongshi_Stall_Time++;
    ...
    if ((Test.Status_Ing == Test_Station_Busy) && (bug_count > 60)) {
        ...
        if ((bug_count) > 100) {
            qDebug() << "w_Test::Pri_Test_Time_Stop";
            bug_count = 0;
            Test_Beep_State(Beep.NG_Time, "NG");
            Sigtest_Usart_Send(QString("LEDR").toLatin1().data());
            Sigtest_Usart_Send(QString("LEDR").toLatin1().data());
...
void w_Test::Test_Mag_MagDir_YY(QString str, int mag_rotation)
{
    uint16_t i = 0, j = 0;
    int16_t  temp = 0;
    ...
    if (str == QString(tr("one"))) {
        ...
        for (i = 10; i < 390; i++) { // -波形差积-410改为390
            temp = (TaskMagDirstrList[3][i].toInt()) - 0x80;
            area1 += temp*temp;
            temp = (TaskMagDirstrList[4][i].toInt()) - 0x80;
            area2 += temp*temp;
            // 差积
            temp = (TaskMagDirstrList[3][i].toInt() - TaskMagDirstrList[4][i].toInt());
            diff += temp*temp;
            // 移相后的差积
            // area2+=MagBuf.wave[AssistantItem-5][i]*MagBuf.wave[AssistantItem-5][i];
            for (j = 1; j < 11; j++) {
                temp = (TaskMagDirstrList[3][i].toInt() - TaskMagDirstrList[4][i+j].toInt());
                // -副相左移  point
                diff1 += temp*temp;
                temp = (TaskMagDirstrList[3][i].toInt() - TaskMagDirstrList[4][i-j].toInt());
                // -副相右移  pointgreen
        ...
        if (((diff*3 < area1) && (diff*3 < area2)) || \
            ...
            TaskMagDirnumber_YY = 2;
        } else {
            if (diff1 < diff2) { // -副相在前
                if (mag_rotation == 0) {
                    TaskMagDirnumber_YY = 1;
                } else {
                    TaskMagDirnumber_YY = 0;
                }
        ...
        for (i = 10; i < 390; i++) { // -波形差积-410改为390
            temp = (TaskMagDirstrList[4][i].toInt()) - 0x80;
            area1 += temp*temp;
            temp = (TaskMagDirstrList[5][i].toInt()) - 0x80;
            area2 += temp*temp;
            // 差积
            temp = (TaskMagDirstrList[4][i].toInt() - TaskMagDirstrList[5][i].toInt());
            diff += temp*temp;
            // 移相后的差积
            // area2+=MagBuf.wave[AssistantItem-5][i]*MagBuf.wave[AssistantItem-5][i];
            for (j = 1; j < 11; j++) {
                temp = (TaskMagDirstrList[4][i].toInt() - TaskMagDirstrList[5][i+j].toInt());
                // -副相左移  point
                diff1 += temp*temp;
                temp = (TaskMagDirstrList[4][i].toInt() - TaskMagDirstrList[5][i-j].toInt());
                // -副相右移  pointgreen
        ...
        if (((diff*3 < area1) && (diff*3 < area2)) || \
                (area1 < (area2 >> 4)) || (area2 < (area1 >> 4))) {
            TaskMagDirnumber_YY_ROT = 2;
        } else {
            if (diff1 < diff2) { //副相在前
                if (mag_rotation == 0) {
                    TaskMagDirnumber_YY_ROT = 1;
                } else {
                    TaskMagDirnumber_YY_ROT = 0;
                }
            } else if (diff1 > diff2) { //主相在前
        ...
        if (TaskMagDirnumber_YY_ROT != TaskMagDirnumber_YY) {
            qDebug() << "TaskMagDirnumber_ROT" << TaskMagDirnumber_ROT << \
                        "TaskMagDirnumber_YY" << TaskMagDirnumber_YY;
            qDebug() << "测试转向---不合格";
            TaskMagDirnumber_YY = strOPP.at(6).toInt() + 1;
            TaskMagDirnumber_YY = TaskMagDirnumber_YY%2;
        } else {
            qDebug() << "TaskMagDirnumber_YY_ROT" << TaskMagDirnumber_YY_ROT \
                     << "TaskMagDirnumber_YY" << TaskMagDirnumber_YY;
            qDebug() << "测试转向---合格";
        }
        TaskMagDirstrList[3].clear();
...
void w_Test::Test_Delete_Row(int value)
{
    ...
    qDebug() << "w_Test::Test_Delete_Row----------" << value << Test.Status_Ing;
    int PG_True = 0;
    switch (value) {
    case 1: // -电阻
        if ((DLR_UpDown.at(20).toDouble() != 0.000) && (DCR_Total_Number >= 3)) {
            // -包含电阻不平衡度
            if ((DCR_Total_Number >= 6) && (YY_Model)) {
                strResult = strResult.mid(0, strResult.size() - DCR_Total_Number - 2);
                StrResult_Size = strResult.size();
                strJudge = strJudge.mid(0, strJudge.size() - DCR_Total_Number - 2);
                StrJudge_Size = strJudge.size();
                Test.Current_Row = Test.Current_Row - DCR_Total_Number - 2;
            } else {
    ...
    case 2: // -反嵌
        if (TaskMagDirnumber != 5) { // -包含转向
            if (YY_ROT_Test) {
                strResult = strResult.mid(0, strResult.size() - MAG_Total_Number - 2);
                StrResult_Size = strResult.size();
                strJudge = strJudge.mid(0, strJudge.size() - MAG_Total_Number - 2);
                StrJudge_Size = strJudge.size();
                Test.Current_Row = Test.Current_Row - MAG_Total_Number - 2;
            } else {
...
//        qDebug() << "waveform_all_count--1" << waveform_all_count;
        ...
        waveform_all_count = waveform_all_count - ((FG_Test_Data.at(0).toInt()-1)/3 + 1)*3;
        // -处理波形
        WavePoint_strList[(waveform_row_count-1)*3 + 0].clear();
        WavePoint_strList[(waveform_row_count-1)*3 + 1].clear();
        WavePoint_strList[(waveform_row_count-1)*3 + 2].clear();
...
{
    if (Test.Status_Ing != Test_Station_Ask) {
        qDebug() << "return---> w_Test::Pri_Ask_Test_Current()";
        return;
    } else {
        //
    }
...
{
    if (Test.Status_Ing != Test_Station_Ask) {
        qDebug() << "return---> w_Test::Pri_Ask_Test_Exit()";
        return;
    } else {
        //
    }
...
void w_Test::Test_picture() {
    if (Testing_picture) {
        return;
    }
    Testing_picture = true;
    ...
    QString fileName;
    sprintf(str, "./%02d-%02d-%02d-", t.tm_year, t.tm_mon, t.tm_mday);
    fileName = str;
    sprintf(str,"%02d-%02d-%02d.png",t.tm_hour,t.tm_min,t.tm_sec);
    fileName = fileName + str;

    QString fileName_cut;
    fileName_cut = fileName.mid(2, fileName.size());
    qDebug() << "fileName_cut" << fileName_cut;
    ...
    QDir *temp = new QDir;
    bool exist = temp->exists("/mnt/sdcard/picture");
    if (exist) {
        //
    } else {
        system("mkdir /mnt/sdcard/picture");
    }
    sprintf(copystr, "mv /mnt/%s /mnt/sdcard/picture", \
            QString(fileName_cut).toUtf8().data());
    system(copystr);

    Testing_picture = false;
    QWSServer::setCursorVisible(true);//隐藏鼠标
}
...
void w_Test::Test_Background_Param(QStringList list)
{
    ...
    scan_scart = list.at(8).toInt();
    scart_stop = list.at(9).toInt();
}
...
QStringList w_Test::Pub_Set_PWR(can_frame frame)
{
    ...
    can_data_2 = 0;
    can_data_3 = 0;
    can_data_1 = (frame.data[1]*256 + frame.data[2]);
    can_data_2 = (frame.data[3]*256 + frame.data[4]);
    can_data_3 = (frame.data[5]*256 + frame.data[6]);

    if (frame.data[0] == 1) {
        Volt_A    = (can_data_1)/10;
        Current_A = (can_data_2)/1000; // -电流
        Pwr_A     = (can_data_3)/10;   // -功率
    } else if (frame.data[0] == 2) {
        Freq_A = (can_data_1)/10;
        time   = frame.data[3]*256+frame.data[4];
        Dir    =  frame.data[5];
        Speed  = frame.data[6]*256+frame.data[7];
    } else if (frame.data[0] == 3) {
        Volt_B    = (can_data_1)/10;   // -容压
        Current_B = (can_data_2)/1000;
        Pwr_B     = (can_data_3)/10;
    } else {
        //
    }
...
QStringList w_Test::Pub_Set_PG(can_frame frame)
{
    ...
    if (frame.data[0] == 1) {     // -电流,低电平,高电平
        pg_Cur = (can_data_1)/100;
        pg_floor = (can_data_2)/100;
        pg_up = (can_data_3)/100;
    } else if (frame.data[0] == 2) { // -频率,min avr max
        pg_freq_min = ((can_data_1))/10;
        pg_freq_max = ((can_data_2))/10;
        pg_freq_avr = ((can_data_3))/10;
    } else if (frame.data[0] == 3) { // -占空比,min avr max
        pg_duty_min = ((can_data_1))/10;
        pg_duty_max = ((can_data_2))/10;
        pg_duty_avr = ((can_data_3))/10;
    } else {
        //
    }
    QStringList data;
...
void w_Test::Pub_Main_can_to_test(can_frame canframe, int value)
{
    ...
    case 1:
        if (Conf_Item[Test.Finshed_Count].toInt() == 1) {
            ...
            if (canframe.data[0] == 1) {
                can_data_force = (canframe.data[1]*256 + canframe.data[2]);
                Turn_Elec_force = (can_data_force)/10;
                Turn_Elec_force_List.append(QString::number(Turn_Elec_force));
            } else {
                //
            }
            ...
            if (canframe.data[0] < 10) { // -功率
                ...
                if ((canframe.data[0] == 2) && (PWR_Back_List != QStringList())) {
                    ...
                    if (PWR_Data_Up > 5) {
                        PWR_Data_Up = 0;
                        emit WriteMessage(6000, CMD_Put_PWR, QString(PWR_Back_List.join(" ")).toUtf8());
                    } else {
                        //
                    }
                } else {
            ...
            if (canframe.data[0] == 1) { // -只获取A相
                can_data_I = (canframe.data[3]*256 + canframe.data[4]);
                can_data_P = (canframe.data[5]*256 + canframe.data[6]);
                PWR_Back_List.append(QString::number((can_data_I)/1000, 'f', 3)); // -电流
                PWR_Back_List.append(QString::number(((can_data_P))/10, 'f', 1));   // -功率
    ...
    case 3:
        if (Test.Finshed_Count >= Conf_Item.size()) {
            qDebug() << "return---> w_Test::Pub_Main_can_to_test PG Error Back";
            return;
        } else {
            //
        }
...
void w_Test::Test_Get_Temp(QString list)
{
    ...
    if (Temp_Get < 150) {
        Temp_Error = false;
        test_parameters[3]->setText(QString(tr("温度:")) + \
                                    QString::number(Temp_Get, 'f', 1) + "°C");
    } else {
        Temp_Error = true;
        test_parameters[3]->setText(tr("温度:--°C"));
    }
    WriteMessage(6000, CMD_Put_76, test_parameters[3]->text().toUtf8());
}
...
void w_Test::join_key_out()
{
    ...
    if ((scan_scart == 0) && (scart_stop == 0)) {
        ...
        ui->label_number->setText(QString(tr("编号:")) + \
                                  keyboard_string.mid\
                                  (scan_scart, keyboard_string.size() - scan_scart));
...
void w_Test::Pub_MAG_Param(QStringList mag_data, int *waveform_point)
{
    ...
    if (strOPP.at(6).toInt() != 2) { // -判断磁旋不是 不测
        if ((mag_back_number == 2) || (mag_back_number == 1) || (mag_back_number == 0)) {
            ...
            if (YY_ROT_Test) {
                if ((mag_back_number == 5) || (mag_back_number == 4) || (mag_back_number == 3)) {
                    for (i = 0; i < 400; i++) {
                        TaskMagDirstrList[mag_back_number].append(QString::number(waveform_point[i]));
                    }
                }
        ...
        if ((mag_back_number == 4) && (YY_ROT_Test)) {
            if (((strOPP.at(NUM_OPP*3+3).toInt()) == 2) && \
                    ((strOPP.at(NUM_OPP*4+3).toInt()) == 2)) {
                Test_Mag_MagDir_YY(QString(tr("one")), mag_set_rotation); // -测试磁旋
            }
        } else {
            //
        }
        if ((mag_back_number == 5) && (YY_ROT_Test)) {
            if (((strOPP.at(NUM_OPP*4 + 3).toInt()) == 2) && \
                    ((strOPP.at(NUM_OPP*5 + 3).toInt()) == 2)) {
                Test_Mag_MagDir_YY(QString(tr("two")), mag_set_rotation); // -测试磁旋
            }
        } else {
            //
        }
...
void w_Test::Pub_IMP_Param(QStringList imp_data, int *waveform_point, int *waveform_point_compare )
{
    ...
    if (strITT.at(16).toInt() == 2) {
        ...
        if ((imp_back_number == 0) || (imp_back_number == 3)) {
            if (Station_Chose == 0x13) {
                for (i = 0; i < 400; i++) {
    ...
    if (imp_waveform_error) {
        qDebug() << "w_Test::Pub_IMP_Param imp_waveform_error" << imp_waveform_error << imp_count;
        WavePoint_strList[(waveform_row_count-1)*3 + waveform_all_count%3].append \
...
{
    qDebug() << "w_Test::Pub_FG_Param()" << FG_shape.size() << FG_shape.at(0);
...
void w_Test::FG_Param(QStringList shape_list) // int *waveform_point
{
    ...
    if (FG_Test_Data.at(0).toInt() == FG_Shape_Count) {
        ...
        qDebug() << "w_Test::FG_Param waveform is finsh";
    } else {
        //
    }
}
...
void w_Test::PG_Param(QStringList pg_data, int *waveform_point, int *waveform_point_c)
{
    ...
    qDebug() << "w_Test::PG_Param" << pg_waveform_error;
    ...
    if ((Ini_Motor == QString("PG")) && \
        ...
        qDebug() << "return---> w_Test::PG_Param()";
        return;
    }

    qDebug() << "w_Test::PG_Param()";
    if (waveform_all_count%3 == 0) {
        for (i = 0; i < 3; i++) { // -3组数据清理
            wave[i]->Test_Shape = 0;
            wave[i]->num = 0;
    ...
    if (pg_waveform_error) { //-采集波形有错误
        qDebug() << "w_Test::PG_Param waveform is error";
        ...
        qDebug() << "w_Test::PG_Param waveform is right";
    }
    /*波形显示*/
    for (i = 0; i < 400; i++) {
        wave[waveform_all_count%3]->point[i] = 260;
    ...
    if (PWR_Test_Data.at(0).toInt() == (PWR_Test_Count+1)) {
        ...
        qDebug() << "w_Test::PG_Param waveform is finsh";
    } else {
        //
    }
}
...
void w_Test::PC_Test_Result_ItemData(QString netdata, QString param)
{
    ...
    emit WriteMessage(6000, CMD_Put_Start_Item, doc.toByteArray());
}
