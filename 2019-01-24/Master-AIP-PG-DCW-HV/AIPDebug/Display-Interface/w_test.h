...
public:
    ...
    QStringList ACWgroup_num;                     // -相间交流耐压组号
    int  ACW_Test_Count, ACW_Test_Total;           // -相间交流耐压测试数目 相间交流耐压测试总数
    QStringList DCWgroup_num;                     // -相间直流耐压组号
    int  DCW_Test_Count, DCW_Test_Total;           // -相间直流耐压测试数目 相间直流耐压测试总数
    ...
    int  IR_First_Test_Enabled;
    int  ACW_First_Test_Enabled;
...
private:
    ...
    int  TaskMagDirnumber_ME;
    int  ME_Count;
    int  TaskMagDirnumber_ROT;        // 磁旋标记 正转 反转 不转

    int  TaskMagDirnumber_YY;            // -YY磁旋标记(正转-反转-不转)
    int  TaskMagDirnumber_YY_ROT;        // YY磁旋标记 正转 反转 不转

    int  MAG_Current_Testing_Item;    // -反嵌当前测试中项目
    QStringList TaskMagDirstrList[6]; // -反嵌测试列表
    ...
    QStringList ACW_Error;         // -相间交流耐压异常错误
    QStringList DCW_Error;         // -相间直流耐压异常错误
    ...
    QTimer *Noise_Blink; // noise--
    int count_Noise_Blink;
...
//    int  Ini_NoBalance;   // -电阻不平衡度测试
    ...
    bool Turn_Elec_forceing;
    QStringList Turn_Elec_force_List;
    int  BEMF_Count;
    ...
    bool NetTest_Ready_DCW;      bool DCW_Issue_Ing;
    ...
    bool NetTest_Resdy_PWR;      bool PWR_Issue_Ing;
    bool NetTest_Resdy_HV;       bool HV_Issue_Ing;

    void Can_Start_DCR_Beforce();
    void Can_Start_DLR();
    void Can_Test_MAG_Parameter();
    void Can_Start_MAG();
    void Can_Start_IR(int);
    void Can_Start_ACW(int);
    void Can_Start_DCW();
    void Can_Start_IMP();
    ...
    void Test_Mag_MagDir_YY(QString, int);
    ...
    void Test_Net_WMessage(QString data);
    ...
    void Board_DCW_Data(QStringList); // -直耐测试数据
    ...
    void Board_INDL_Unbalance(int);   // -电感不平衡度测试
    ...
    void Board_HV_Data(QStringList);  // -高压测试数据
    ...
    void Pri_Test_Time_Stop(QString, int);
    void TTT();
    void Test_Histogram_Widget();
    void Test_Ask_Widget();

    int  Ciximinghua_Old;
    void Test_CiXiMingHua();

    QLabel *COM_label;
    void  ICO_Show_Widget();

    QString ACW_Result_DeChang;
    bool  W_Connext;
    int   Ini_W_Connext;
    int   Ini_DeChang;
    int   Ini_Discharge;
    int   DeChang_Connect_Time;
    int32_t DC_IO_Put;
    void  Dechang_Connect(int);
    ...
    QTimer *WL_Start;
    void Test_WL_Hall();
    void Test_WL_Hall_FInsh();
    quint16  crc16(QByteArray);
    uint16_t crc16_modbus(char *data, uint16_t length);

    bool mag_hall_rotation;
    int PWR_Data_Up;

    QWidget *Prompt_Widget;
    QLabel  *Prompt_Up_label;
    void  Prompt_Windows();
    void Test_New_Hall();

    QTimer *PG_Start_Start;
    int     PG_Start_count;

    bool Testing_picture;
    void Test_picture();
    void PLC_test_to_Switchover(int, int32_t, int);

signals:
    void Sigtest_Usart_Send(char *Buf);
    void Sigtest_Usart_Send_DeChang(QString);
    void Sigtest_Usart_Send_WEILING(QString);
...
private slots:
    ...
    void Noise_Blink_timeout();
    void WL_Start_timeout();
    void PG_State_timeout();
